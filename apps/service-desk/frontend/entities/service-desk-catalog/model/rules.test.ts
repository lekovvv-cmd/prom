...
describe("service desk form rules", () => {
  ...
  it("prefers resolved dictionary options", () => {
    const options = getFieldOptions({
      options: [{ value: "old" }],
      effective_options: [{ value: "new", label: "Новое" }],
    });
    expect(options).toEqual([{ value: "new", label: "Новое" }]);
    expect(getFieldOptions({ options: [{ value: "fallback" }] })).toEqual([
      { value: "fallback" },
    ]);
  });
  ...
  it("supports empty and non-empty dynamic conditions", () => {
    expect(
      matchesRules(
        { field: "details", operator: "is_empty" },
        { details: "" },
        false,
      ),
    ).toBe(true);
    expect(
      matchesRules(
        { field: "details", operator: "is_not_empty" },
        { details: "Описание" },
        false,
      ),
    ).toBe(true);
  ...
  it("computes conditional visibility and required state", () => {
    ...
    expect(
      isFieldVisible(
        { visibility_rules: { field: "kind", value: "exam" } },
        values,
      ),
    ).toBe(true);
    expect(
      isFieldVisible(
        { visibility_rules: { field: "kind", value: "other" } },
        values,
      ),
    ).toBe(false);
    expect(
      isFieldRequired(
        {
          is_required: false,
          required_rules: { field: "kind", value: "exam" },
        },
        values,
      ),
    ).toBe(true);
  ...
  it("normalizes renderer values and maps controls", () => {
    ...
    expect(normalizeFieldValue("multiselect", [1, "two"])).toEqual([
      "1",
      "two",
    ]);
