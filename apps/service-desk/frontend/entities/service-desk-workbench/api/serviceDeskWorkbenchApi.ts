...
export function getWorkbenchTickets(params: URLSearchParams) {
  return serviceDeskApiClient.request<WorkbenchPage>(
    `/workbench/tickets?${params}`,
  );
}
...
export function getWorkbenchUsers(eligibleAssignees = false) {
  return serviceDeskApiClient.request<WorkbenchUserOption[]>(
    `/workbench/users?eligible_assignees=${eligibleAssignees}`,
  );
}
...
export function getWorkbenchServices(categoryId = "") {
  return serviceDeskApiClient.request<CatalogOption[]>(
    `/services${categoryId ? `?category_id=${categoryId}` : ""}`,
  );
}
export function performWorkbenchAction(
  ticketId: string,
  action: string,
  ...
  if (action === "change_priority") {
    return serviceDeskApiClient.request(`/tickets/${ticketId}/priority`, {
      method: "PATCH",
  ...
  return serviceDeskApiClient.request(endpoint, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
