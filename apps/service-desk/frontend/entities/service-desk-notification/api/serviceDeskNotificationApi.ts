...
import type {
  ServiceDeskContextualCounters,
  ServiceDeskNotification,
} from "../model/types";

export function getServiceDeskNotifications(limit = 30) {
  return serviceDeskApiClient.request<ServiceDeskNotification[]>(
    `/notifications?limit=${limit}`,
  );
}

export function getServiceDeskUnreadCount() {
  return serviceDeskApiClient.request<{ count: number }>(
...
export function markAllServiceDeskNotificationsRead() {
  return serviceDeskApiClient.request<{ marked_read: number }>(
...
export function getServiceDeskContextualCounters() {
  return serviceDeskApiClient.request<ServiceDeskContextualCounters>(
