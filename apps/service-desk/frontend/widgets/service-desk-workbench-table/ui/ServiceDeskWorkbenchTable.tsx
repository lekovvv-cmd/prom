...
const actionLabels: Record<ServiceDeskAllowedAction, string> = {
  ...
  wait_external: "Ожидать внешнее действие",
  resume: "Продолжить",
  resolve: "Выполнить",
  close: "Закрыть",
  cancel: "Отменить",
...
export function ServiceDeskWorkbenchTable({
  items,
  onAction,
}: {
  items: WorkbenchTicket[];
  onAction: (ticket: WorkbenchTicket, action: ServiceDeskAllowedAction) => void;
}) {
  return (
    <div className="service-desk-workbench-table-shell">
      <Table>
        <table className="service-desk-workbench-table">
          <thead>
            <tr>
              <th>Номер</th>
              <th>Заявка</th>
          ...
          <tbody>
            {items.map((ticket) => (
              <tr key={ticket.ticket_id}>
                <td>
                  <Link to={`/service-desk/tickets/${ticket.ticket_id}`}>
                    {ticket.number ?? "Черновик"}
                  </Link>
                </td>
                <td>
                  <strong>{ticket.title}</strong>
                  <span>{ticket.service.title}</span>
                </td>
                ...
                <td data-testid="sla-cell">
                  <WorkbenchSlaIndicator sla={ticket.sla} />
                </td>
                <td>{formatDateTime(ticket.created_at)}</td>
                <td>{formatDateTime(ticket.updated_at)}</td>
                <td>
                  {ticket.allowed_actions.length ? (
                    <Select
                      aria-label={`Действия ${ticket.number ?? ticket.title}`}
                      value=""
                      onChange={(event) => {
                        const action = event.target
                          .value as ServiceDeskAllowedAction;
                        if (action) onAction(ticket, action);
                      }}
                    >
                      <option value="">Выберите</option>
                      {ticket.allowed_actions.map((action) => (
      ...
      <p className="workbench-table-scroll-hint">
