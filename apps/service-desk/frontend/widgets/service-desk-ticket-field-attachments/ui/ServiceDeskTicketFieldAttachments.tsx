...
export function ServiceDeskTicketFieldAttachments({
  ticketId,
  fields,
  canDelete = false,
}: {
    ...
    Promise.all(
      fileFields.map(
        async (field) =>
          [
            field.key,
            await listServiceDeskFieldAttachments(ticketId, field.key),
          ] as const,
      ),
    )
      .then((items) => {
        if (active) setAttachments(Object.fromEntries(items));
      })
      .catch(() => undefined);
    return () => {
      active = false;
  ...
  if (!fileFields.some((field) => (attachments[field.key] ?? []).length))
    return null;
  ...
  async function remove(fieldKey: string, attachment: ServiceDeskAttachment) {
  ...
  return (
    <Card>
      <h3>
        <Paperclip size={16} aria-hidden="true" /> Файлы полей формы
      </h3>
      <div className="admin-config-list">
        {fileFields.flatMap((field) =>
          (attachments[field.key] ?? []).map((attachment) => (
            <div className="admin-config-row" key={attachment.id}>
              <span>
                <strong>
                  {field.label}: {attachment.file_name}
                </strong>
                <small>
              ...
              <div className="table-actions">
                ...
                {canDelete ? (
                  <button
                    type="button"
                    className="button button-secondary"
                    onClick={() => void remove(field.key, attachment)}
                  >
                    <Trash2 size={14} />
                    Удалить
                  </button>
