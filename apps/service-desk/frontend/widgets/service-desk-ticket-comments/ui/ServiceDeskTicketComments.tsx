...
import {
  deleteServiceDeskComment,
  downloadServiceDeskAttachment,
  listServiceDeskCommentAttachments,
  updateServiceDeskComment,
  uploadServiceDeskCommentAttachment,
...
export function ServiceDeskTicketComments({
  ...
  async function saveEdit(commentId: string) {
    ...
    try {
      await updateServiceDeskComment(ticket.id, commentId, editingBody.trim());
      setEditingId(null);
      setEditingBody("");
      await onTicketChanged();
    } catch (requestError) {
  ...
  async function removeComment(commentId: string) {
    ...
    try {
      await deleteServiceDeskComment(ticket.id, commentId);
      await onTicketChanged();
    } catch (requestError) {
      setError(
        requestError instanceof Error
  ...
  return (
    <Card className="service-desk-comments">
          ...
          {ticket.comments.map((comment) => (
            ...
            >
              <div className="service-desk-comment-meta">
                ...
                {comment.visibility === "internal" ? (
                  <span className="service-desk-comment-visibility">
                    <Lock size={12} /> Внутренний
                  </span>
                ) : null}
              </div>
              {editingId === comment.id ? (
                <div className="service-desk-comment-form">
                  <Textarea
                    label="Комментарий"
                    value={editingBody}
                  ...
                  <div className="button-row">
                    <Button onClick={() => void saveEdit(comment.id)}>
                      Сохранить
                    </Button>
              ...
              />
              {comment.updated_at ? (
                <span className="muted">Изменён</span>
              ) : null}
            </li>
          ))}
        </ol>
      ...
      {isLocked ? (
        <p className="muted">
          В закрытую или отменённую заявку комментарии добавить нельзя.
        </p>
      ) : (
          ...
          {canAddInternal ? (
            ...
            >
              ...
              <option value="internal">
                Внутренний — только для Service Desk
              </option>
            </Select>
          ) : null}
...
function CommentAttachments({
  ...
  const [error, setError] = useState<string | null>(null);
  useEffect(() => {
    void listServiceDeskCommentAttachments(ticketId, commentId)
      .then(setAttachments)
      .catch(() => undefined);
  }, [commentId, ticketId]);
  async function upload(file: File) {
    try {
      const attachment = await uploadServiceDeskCommentAttachment(
        ticketId,
        commentId,
        file,
      );
  ...
  return (
    <div className="comment-attachments">
      ...
      {canUpload ? (
        <label className="comment-attachment-upload">
