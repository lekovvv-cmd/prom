...
describe("ContextualCounterList", () => {
  it("renders actor counters and hides unavailable counters", () => {
    const html = renderToStaticMarkup(
      <ContextualCounterList
        counters={{
          waiting_my_approval: 2,
          assigned_to_me: 1,
          awaiting_my_response: 0,
          sla_breaches: null,
        }}
      />,
    );
  ...
  it("links every available counter to its matching workbench view", () => {
    const html = renderToStaticMarkup(
      <MemoryRouter>
        <ContextualCounterList
          interactive
    ...
    expect(html).toContain(
      "/service-desk/workbench?quick_view=waiting_approval",
    );
    expect(html).toContain("/service-desk/workbench?quick_view=assigned_to_me");
    expect(html).toContain(
      "/service-desk/workbench?quick_view=waiting_requester",
    );
