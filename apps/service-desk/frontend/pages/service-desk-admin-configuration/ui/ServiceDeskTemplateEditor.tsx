...
import type {
  ...
  ServiceDeskTemplateFieldType,
} from "../../../entities/service-desk-catalog/model/types";
import {
  createAdminTemplateField,
  createAdminTemplateVersion,
  deleteAdminTemplateField,
  listAdminDictionaries,
  listAdminServices,
  listAdminTemplateVersions,
  previewAdminTemplateVersion,
  publishAdminTemplateVersion,
  reorderAdminTemplateFields,
  updateAdminTemplateField,
  updateAdminTemplateVersion,
...
import { createSystemSlug } from "../../../lib/createSystemSlug";
...
type FieldReference = {
  key: string;
  label: string;
  fieldType: ServiceDeskTemplateFieldType;
};
...
const fieldTypes: ServiceDeskTemplateFieldType[] = [
  "text",
  "textarea",
  "rich_text",
  "select",
...
export function ServiceDeskTemplateEditor() {
  ...
  const fieldReferences = useMemo<FieldReference[]>(
    () =>
  ...
  useEffect(() => {
    listAdminServices()
      .then(setServices)
      .catch((reason) =>
        setError(errorText(reason, "Не удалось загрузить услуги")),
      );
    listAdminDictionaries()
    ...
    try {
      ...
      if (editingId) {
        ...
        setVersions((current) =>
          current.map((version) =>
            version.id === selected.id
              ? { ...version, fields: [...version.fields, created] }
              : version,
          ),
        );
      }
      ...
      setVersions((current) =>
        current.map((version) =>
          version.id === selected.id
            ? {
                ...version,
                fields: version.fields.filter((field) => field.id !== fieldId),
              }
            : version,
        ),
      );
  ...
  async function moveField(index: number, delta: number) {
    ...
    try {
      const result = await reorderAdminTemplateFields(
        selected.id,
        fields.map((field) => field.id),
      );
      setVersions((current) =>
        current.map((version) =>
          version.id === selected.id ? { ...version, fields: result } : version,
        ),
      );
    } catch (reason) {
  ...
  return (
    <PageLayout
      title="Шаблоны форм"
      subtitle="Создавайте форму заявки по шагам: название поля, тип ответа и правила — только если они нужны."
    >
      ...
      <div className="admin-config-grid template-builder-grid">
        <Card className="template-version-sidebar">
          <div className="template-sidebar-heading">
            ...
            <p className="muted">
              У каждой услуги своя форма заявки и история её версий.
            </p>
          </div>
          <Select
            label="Услуга"
            value={serviceId}
            onChange={(event) => void loadVersions(event.target.value)}
          >
          ...
          <div className="template-version-actions">
            ...
            <small className="field-help">
              Новая версия — отдельный черновик. Добавьте в него нужные поля:
              опубликованные заявки не изменятся.
            </small>
          </div>
          {serviceId && !versions.length ? (
            <p className="template-empty-copy">
              У этой услуги ещё нет форм. Создайте первую версию, чтобы добавить
              поля.
            </p>
          ) : null}
          {versions.length ? (
            <div
              className="admin-config-list template-version-list"
              aria-label="Версии формы"
            >
              {versions.map((version) => (
                <button
                  type="button"
                  className={`admin-config-select template-version-select ${version.id === selectedId ? "active" : ""}`}
                  key={version.id}
                  onClick={() => {
                    setSelectedId(version.id);
                    setPreview(null);
        ...
        {selected ? (
          <Card className="template-editor-surface">
            <div className="template-editor-header">
              <div>
                <span
                  className={`template-version-badge is-${selected.status}`}
                >
                ...
                <p className="muted">
                  {selected.status === "draft"
                    ? "Настройте поля, затем опубликуйте версию. В заявках, созданных ранее, форма останется прежней."
                    : "Эта версия уже используется в заявках и доступна только для просмотра."}
                </p>
              </div>
              <span className="button-row">
                ...
                <Button
                  variant="ghost"
                  onClick={() =>
                    void previewAdminTemplateVersion(selected.id)
                      .then((result) => setPreview(result.fields))
                      .catch((reason) =>
                        setError(
                          errorText(reason, "Не удалось открыть предпросмотр"),
                        ),
                      )
                  }
                >
            ...
            >
              <div className="service-desk-section-heading">
                <div>
                  ...
                  <p className="muted">
                    Именно в таком порядке пользователь увидит поля при создании
                    заявки.
                  </p>
                </div>
              </div>
              {!selected.fields.length ? (
                <p className="template-empty-copy">
                  Добавьте первое поле выше. Например, «Тема обращения» или
                  «Дата занятия».
                </p>
              ) : null}
              <div className="admin-config-list">
                {selected.fields.map((field, index) => (
                  <div
                    className="admin-config-row template-field-row"
                    key={field.id}
                  >
                    <span>
                      <strong>
                    ...
                    {selected.status === "draft" ? (
                      <span className="button-row">
                        ...
                        <Button
                          variant="ghost"
                          aria-label={`Поднять поле «${field.label}»`}
                        ...
                        <Button
                          variant="ghost"
                          aria-label={`Опустить поле «${field.label}»`}
                          onClick={() => void moveField(index, 1)}
          ...
          <Card className="template-empty-state">
            <h3>Начните с услуги</h3>
            <p className="muted">
              Слева выберите услугу и версию формы. Для новой услуги сначала
              создайте черновую версию.
            </p>
          </Card>
        )}
      </div>
...
function VersionSettings({
  ...
  useEffect(() => {
    getServiceDeskUserOptions("service_desk.be_assignee")
      .then(setAssignees)
      .catch(() => setAssignees([]));
  }, []);
  ...
  async function save() {
    onSaved(
      await updateAdminTemplateVersion(version.id, {
        system_settings: {
          default_title: defaultTitle || null,
          is_title_editable: titleEditable,
          is_description_required: descriptionRequired,
  ...
  return (
    <Card className="template-version-settings">
      <div className="template-section-title">
        <span className="template-step-number">Общее</span>
        <div>
          <h3>Настройки всей формы</h3>
          <p className="muted">
            Эти параметры относятся ко всей заявке, а не к отдельному полю.
          </p>
        </div>
      </div>
      <div className="template-step-grid">
        <div className="template-control-with-help">
          <Input
            label="Тема заявки по умолчанию"
            value={defaultTitle}
            onChange={(event) => setDefaultTitle(event.target.value)}
          />
          <small className="field-help">
            Подставится в новую заявку, если пользователь её не изменит.
          </small>
        </div>
        <div className="template-control-with-help">
          <Input
            label="Подсказка в начале формы"
            value={helpText}
            onChange={(event) => setHelpText(event.target.value)}
          />
          <small className="field-help">
            Коротко объясните, что нужно приложить или указать в заявке.
          </small>
        </div>
        <label className="checkbox-field template-choice">
          <input
            type="checkbox"
            checked={titleEditable}
            onChange={(event) => setTitleEditable(event.target.checked)}
          />
          <span>
            <strong>Разрешить менять тему заявки</strong>
            <small>Пользователь сможет уточнить тему перед отправкой.</small>
          </span>
        </label>
        <label className="checkbox-field template-choice">
          <input
            ...
            onChange={(event) => setDescriptionRequired(event.target.checked)}
          />
          <span>
            <strong>Сделать описание заявки обязательным</strong>
            <small>Без описания заявку нельзя будет отправить.</small>
          </span>
        </label>
        <div className="template-control-with-help">
          <Select
          ...
          <small className="field-help">
            При необходимости этот исполнитель получит новую заявку сразу после
            отправки.
          </small>
        </div>
      </div>
...
function FieldEditor({
  ...
  const hasValidation = [
    ...
    "email",
    "file",
  ].includes(draft.field_type);
  const title = editingId ? "Редактирование поля" : "Добавьте поле";

  return (
    <Card className="template-field-editor">
      <div className="template-editor-intro">
        <div>
          <h3>{title}</h3>
          <p>
            Сначала назовите поле так, как его увидит пользователь. Настройки
            ниже можно добавлять постепенно.
          </p>
        </div>
        {editingId ? (
      ...
      <section
        ...
        aria-labelledby="field-basics-heading"
      >
        <div className="template-section-title">
          <span className="template-step-number">1</span>
          <div>
            <h4 id="field-basics-heading">Что спросить у пользователя</h4>
            <p className="muted">Название отображается в форме заявки.</p>
          </div>
        </div>
        <div className="template-step-grid">
          <div className="template-control-with-help">
            <Input
              label="Название поля для пользователя"
              value={draft.label}
              placeholder="Например, дата занятия"
              onChange={(event) =>
                update({
                  label: event.target.value,
                  key: editingId
                    ? draft.key
                    : createSystemSlug(event.target.value),
                })
              }
            />
            <small className="field-help">
              Пишите простыми словами: «Номер аудитории», «ФИО преподавателя»,
              «Дата занятия».
            </small>
          </div>
        </div>
      ...
      <section
        className="template-editor-step"
        aria-labelledby="field-type-heading"
      >
        <div className="template-section-title">
          <span className="template-step-number">2</span>
          <div>
            <h4 id="field-type-heading">Какой ответ ожидать</h4>
            <p className="muted">
              Тип определяет, какое поле увидит пользователь и как мы проверим
              ответ.
            </p>
          </div>
        </div>
        <div className="template-step-grid">
          <div className="template-control-with-help">
              ...
              onChange={(event) =>
                update({
                  field_type: event.target
                    .value as ServiceDeskTemplateFieldType,
                })
              }
            >
              {fieldTypes.map((type) => (
            ...
            <small className="field-help">
              Выберите «Список» для готовых вариантов, «Файл» для вложений,
              «Дата и время» для расписания.
            </small>
          </div>
          <div className="template-control-with-help">
            <Input
            ...
            />
            <small className="field-help">
              Серый пример внутри поля; он не отправляется как ответ.
            </small>
          </div>
          <div className="template-control-with-help template-step-wide">
            <Input
              label="Подсказка для пользователя (необязательно)"
              value={draft.help_text}
              placeholder="Укажите дату и время по расписанию"
              onChange={(event) => update({ help_text: event.target.value })}
            />
            <small className="field-help">
              Появится под полем и поможет заполнить его без лишних вопросов.
            </small>
          </div>
          {isChoice ? (
            <ChoiceSource
        ...
        {hasValidation ? (
          <ValidationSettings draft={draft} onChange={update} />
        ) : null}
      </section>
      ...
      >
        <div className="template-section-title">
          <span className="template-step-number">3</span>
          <div>
            <h4 id="field-rules-heading">Когда спрашивать это поле</h4>
            <p className="muted">
              Правила необязательны. Оставьте их выключенными, если поле нужно
              всегда.
            </p>
          </div>
        </div>
        <label className="checkbox-field template-choice template-required-choice">
          <input
            type="checkbox"
          ...
          />
          <span>
            <strong>Это поле обязательно всегда</strong>
            <small>
              Пользователь не сможет отправить заявку, пока не заполнит это
              поле.
            </small>
          </span>
        </label>
        <ConditionSection
          kind="visibility"
          rules={draft.visibility_rules}
          fields={fields}
          ownKey={draft.key}
          onChange={(rules) => update({ visibility_rules: rules })}
        />
        <ConditionSection
          kind="required"
          rules={draft.required_rules}
          fields={fields}
          ownKey={draft.key}
          onChange={(rules) => update({ required_rules: rules })}
        />
      </section>
      ...
      <div className="template-editor-footer">
        <div>
          <Button
            onClick={onSave}
            disabled={!generatedKey || !draft.label.trim()}
          >
            <Save size={15} />
            Сохранить поле
          </Button>
        ...
        {!generatedKey || !draft.label.trim() ? (
          <small className="field-help">
            Чтобы сохранить поле, заполните его название.
          </small>
        ) : null}
      </div>
    </Card>
...
function ChoiceSource({
  ...
  return (
    <>
      <div className="template-control-with-help">
        <Select
          label="Откуда брать варианты"
          value={draft.options_mode}
          onChange={(event) =>
            onChange({
              options_mode: event.target.value as "static" | "dictionary",
            })
          }
        >
          <option value="static">Ввести варианты вручную</option>
          <option value="dictionary">Взять из справочника</option>
        </Select>
        <small className="field-help">
          Справочник пригодится, если один и тот же список нужен в нескольких
          формах.
        </small>
      </div>
      {draft.options_mode === "static" ? (
        <div className="template-control-with-help">
          <Input
          ...
          />
          <small className="field-help">
            Введите понятные варианты через запятую. Система сама подготовит их
            для сохранения.
          </small>
        </div>
      ) : (
        <div className="template-control-with-help">
          <Select
            label="Справочник"
            value={draft.dictionary_code}
            onChange={(event) =>
              onChange({
                dictionary_code: event.target.value,
                options_text: "",
              })
            }
          >
            ...
            {dictionaries.map((dictionary) => (
              <option key={dictionary.id} value={dictionary.code}>
                {dictionary.title}
              </option>
            ))}
          </Select>
          <small className="field-help">
            Пользователь увидит актуальные варианты из выбранного справочника.
          </small>
        </div>
      )}
    </>
...
function ValidationSettings({
  ...
  return (
    <details className="template-advanced-settings">
      ...
      <p className="muted">
        Их можно не заполнять. Используйте ограничения, только если форма должна
        проверить ответ автоматически.
      </p>
      <div className="template-step-grid">
        ...
        {["text", "textarea", "rich_text", "email"].includes(
          draft.field_type,
        ) ? (
          <>
            <Input
          ...
          <>
            <div className="template-control-with-help">
              <Input
                ...
                onChange={(event) =>
                  onChange({ allowed_extensions: event.target.value })
                }
              />
              <small className="field-help">
                Перечислите через запятую. Оставьте пустым, чтобы разрешить
                любые файлы.
              </small>
            </div>
            <Input
              ...
              onChange={(event) => onChange({ max_files: event.target.value })}
            />
          </>
        ) : null}
      </div>
...
function ConditionSection({
  ...
  const label = isVisibility
    ? "Показывать это поле только при выполнении условия"
    : "Сделать это поле обязательным только при выполнении условия";
  ...
  return (
    <section
      className="template-condition-section"
      aria-label={
        isVisibility ? "Условия показа поля" : "Условия обязательности поля"
      }
    >
      <label className="checkbox-field template-choice">
        <input
          type="checkbox"
          checked={enabled}
          disabled={!availableFields.length && !enabled}
          onChange={(event) =>
            event.target.checked ? addRule() : onChange([])
          }
        />
        <span>
          <strong>{label}</strong>
      ...
      {!availableFields.length && !enabled ? (
        <p className="template-condition-hint">
          Сначала сохраните хотя бы одно другое поле. Тогда можно будет выбрать,
          от какого ответа зависит это поле.
        </p>
      ) : null}
      {enabled ? (
        <div className="template-condition-editor">
          <p className="template-condition-summary">
            Если добавите несколько условий, должны выполниться все. Обычно
          ...
          {rules.map((rule, index) => {
            ...
            const isUnknownField =
              Boolean(rule.field) &&
              !availableFields.some((field) => field.key === rule.field);
            return (
              <div className="template-condition-rule" key={`${kind}-${index}`}>
                <div className="template-condition-rule-heading">
                ...
                <div className="template-step-grid template-condition-grid">
                  <Select
                    label={`Поле для условия ${isVisibility ? "видимости" : "обязательности"} ${index + 1}`}
                  ...
                  >
                    ...
                    {isUnknownField ? (
                      <option value={String(rule.field)}>
                        Недоступное поле
                      </option>
                    ) : null}
                  </Select>
                  <Select
                    label={`Проверка для условия ${index + 1}`}
                    value={operator}
                    onChange={(event) =>
                  ...
                  {!["is_empty", "is_not_empty"].includes(operator) ? (
                    <Input
                      label={`Значение для условия ${index + 1}`}
                ...
                {isUnknownField ? (
                  <p className="field-error">
                    Выберите доступное поле: прежнее поле было удалено или это
                    поле нельзя использовать для самого себя.
                  </p>
                ) : null}
              </div>
            );
...
function TemplatePreview({ fields }: { fields: ServiceDeskTemplateField[] }) {
  ...
  useEffect(() => {
    if (fields.some((field) => field.field_type === "user"))
      getServiceDeskUserOptions()
        .then(setUsers)
        .catch(() => setUsers([]));
  }, [fields]);
  ...
  return (
    <Card className="template-preview-card">
      ...
      <p className="muted">
        Так форма выглядит для пользователя. Условия отображения зависят от
        выбранных ответов.
      </p>
...
function buildPayload(draft: EditorDraft, position: number) {
  ...
  if (draft.allowed_extensions)
    validation.allowed_extensions = draft.allowed_extensions
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean);
  ...
  if (draft.options_mode === "dictionary" && draft.dictionary_code.trim()) {
    ...
    payload.options = draft.options_text
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
      .map((label) => ({ label, value: createSystemSlug(label) }));
  }
...
function fromField(field: ServiceDeskTemplateField): EditorDraft {
  return {
    ...
    options_mode: field.dictionary_code ? "dictionary" : "static",
    ...
    min_length: String(field.validation?.min_length ?? ""),
    max_length: String(field.validation?.max_length ?? ""),
    allowed_extensions: Array.isArray(field.validation?.allowed_extensions)
      ? field.validation.allowed_extensions.join(", ")
      : "",
    max_files: String(field.validation?.max_files ?? ""),
    visibility_rules: toRules(field.visibility_rules),
    required_rules: toRules(field.required_rules),
  };
}
