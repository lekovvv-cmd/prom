  ...
  return (
    <>
      ...
      <div className="metric-grid">
        <Card>
          <span className="muted">SLA первого ответа</span>
          <strong className="metric-value">
            {sla.response_compliance_percent === null
              ? "Нет данных"
              : `${sla.response_compliance_percent}%`}
          </strong>
        </Card>
        <Card>
          <span className="muted">SLA выполнения</span>
          <strong className="metric-value">
            {sla.resolution_compliance_percent === null
              ? "Нет данных"
              : `${sla.resolution_compliance_percent}%`}
          </strong>
        </Card>
        <Card>
          <span className="muted">Нарушения SLA: ответ / выполнение</span>
          <strong className="metric-value">
            {sla.response_breaches} / {sla.resolution_breaches}
          </strong>
        </Card>
        <Card>
          <span className="muted">Рядом с нарушением / нарушены</span>
          <strong className="metric-value">
            {sla.active_near_breach} / {sla.active_breached}
          </strong>
        </Card>
      </div>
      <Distribution
                ...
                {assignees.map((row) => (
                  <tr key={row.user_id}>
                    <td>
                      {row.display_name}
                      {!row.is_active && (
                        <small className="muted"> · доступ отключен</small>
                      )}
                    </td>
      ...
      <Card>
        ...
        <div className="metric-grid approval-metrics">
          <div className="approval-metric">
            ...
            <strong className="metric-value">
              {approvals.pending_approval_stages}
            </strong>
          </div>
          <div className="approval-metric">
            <span>Среднее</span>
            <strong className="metric-value">
              {formatDuration(duration.average_seconds)}
            </strong>
          </div>
          <div className="approval-metric">
            <span>Медиана</span>
            <strong className="metric-value">
              {formatDuration(duration.median_seconds)}
            </strong>
          </div>
          <div className="approval-metric">
            <span>P90</span>
            <strong className="metric-value">
              {formatDuration(duration.p90_seconds)}
            </strong>
          </div>
          <div className="approval-metric">
            <span>Доля отклонений</span>
            <strong className="metric-value">
              {approvals.rejection_rate_percent === null
                ? "Нет данных"
                : `${approvals.rejection_rate_percent}%`}
            </strong>
          </div>
        </div>
      </Card>
...
export function ServiceDeskAdminDashboardPage() {
  ...
  return (
    <>
      <PageLayout
        title="Service Desk - обзор"
        subtitle="Период событий считается по UTC; очередь заявок отражает текущее состояние."
      >
        <Card className="service-desk-dashboard-filter-card">
          <div className="service-desk-dashboard-filter-grid">
            ...
            <Select
              ...
              onChange={(event) =>
                updateFilter("assigneeUserId", event.target.value)
              }
            >
              <option value="">Все</option>
            ...
            <Button
              variant="ghost"
              onClick={() => setFilters(emptyStatsFilters)}
            >
              Сбросить
            </Button>
          </div>
        ...
        {loading ? (
          <Spinner label="Загружаем аналитику" />
        ) : error ? (
          <Card>
