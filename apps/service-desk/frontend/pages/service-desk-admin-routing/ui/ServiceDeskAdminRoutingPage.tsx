...
const CONDITION_OPTIONS: Array<{
  value: ServiceDeskRoutingConditionField;
  label: string;
}> = [
...
const PRIORITY_OPTIONS: Array<{
  value: RoutingActionDraft["priority"];
  label: string;
}> = [
...
function conditionLabel(
  ...
  const fieldLabel =
    CONDITION_OPTIONS.find((item) => item.value === condition.field)?.label ??
    condition.field;
  const fieldKey =
    condition.field === "field_value"
      ? ` · ${condition.field_key ?? "поле не задано"}`
      : "";
  const entityOptions =
    condition.field === "service_id"
      ? serviceOptions
      : condition.field === "category_id"
        ? categoryOptions
        : [];
  const value =
    entityOptions.find((option) => option.value === condition.value)?.label ??
    condition.value;
    ...
    <Card className="service-desk-routing-form-card">
      ...
      <form className="service-desk-routing-form" onSubmit={onSubmit}>
        ...
        <div className="service-desk-routing-order">
          ...
          />
          <p>
            Правила проверяются по возрастанию номера: 1 — первым. После
            сохранения цепочку можно менять стрелками в списке.
          </p>
        </div>
        ...
        <div className="service-desk-routing-form-section">
          <div className="service-desk-routing-card-heading">
            <div>
              <h3>Условия</h3>
              <p>
                Все условия должны совпасть. Пустой набор применяет правило ко
                всем заявкам.
              </p>
            </div>
            <Button
              ...
              {draft.conditions.map((condition, index) => (
                <div
                  className={`service-desk-routing-condition${
                    condition.field === "field_value"
                      ? " service-desk-routing-condition-with-key"
                      : ""
                  }`}
                ...
                >
                  <Select
                    ...
                    onChange={(event) => {
                      const field = event.target
                        .value as ServiceDeskRoutingConditionField;
                      onChange((current) => ({
                        ...current,
                  ...
                  condition.field === "category_id" ? (
                    <SearchableSelect
                      ...
                      searchPlaceholder={
                        condition.field === "service_id"
                          ? "Найдите или выберите услугу"
                          : "Найдите или выберите категорию"
                      }
                      emptyText={
                        condition.field === "service_id"
                          ? "Услуги не найдены"
                ...
                onChange((current) => ({
                  ...
                  action: {
                    ...
                    priority: event.target
                      .value as RoutingActionDraft["priority"],
                  },
                }))
              }
            >
...
export function ServiceDeskAdminRoutingPage() {
    ...
    try {
      ...
      setError(
        requestError instanceof Error
          ...
          : "Не удалось загрузить правила маршрутизации",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const categoryOptions = useMemo(
    () => buildRoutingCategoryOptions(categories),
    [categories],
  );
  const serviceOptions = useMemo(
    () => buildRoutingServiceOptions(services, categories),
    [services, categories],
  );
  ...
  const handleMove = async (ruleId: string, direction: -1 | 1) => {
    ...
    [nextRuleIds[currentIndex], nextRuleIds[nextIndex]] = [
      nextRuleIds[nextIndex],
      nextRuleIds[currentIndex],
    ];
    try {
      setError(null);
      ...
      setError(
        requestError instanceof Error
          ? requestError.message
          : "Не удалось изменить порядок правил",
      );
    }
  };
            ...
            <section className="service-desk-routing-list">
              {rules.length === 0 ? (
                ...
                rules.map((rule, index) => (
                  <Card
                    className="service-desk-routing-rule-card"
                    key={rule.id}
                  >
                    <div className="service-desk-routing-card-heading">
                      <div>
                        <div className="card-topline">
                          <span className="service-desk-routing-priority">
                            Шаг {index + 1} в цепочке
                          </span>
                          <span
                            className={
                              rule.is_active
