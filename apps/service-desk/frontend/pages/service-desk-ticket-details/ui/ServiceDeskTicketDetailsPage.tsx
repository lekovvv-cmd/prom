...
function historyDetails(payload: Record<string, unknown>) {
  for (const key of ["comment", "reason", "resolution_summary", "file_name"])
...
function SlaSummary({ ticket }: { ticket: ServiceDeskTicket }) {
  const firstResponse = ticket.first_response_due_at
    ? ticket.first_response_at
      ? `Выполнен · ${formatDateTime(ticket.first_response_at)}`
      : ticket.is_response_breached
        ? "Срок нарушен"
        : `До ${formatDateTime(ticket.first_response_due_at)}`
    : "Не настроен";
  const resolution = ticket.resolution_due_at
    ? ticket.resolved_at
      ? `Выполнено · ${formatDateTime(ticket.resolved_at)}`
      : ticket.is_resolution_breached
        ? "Срок нарушен"
        : `До ${formatDateTime(ticket.resolution_due_at)}`
    : "Не настроено";
  const paused = ticket.paused_seconds
    ? `${Math.round(ticket.paused_seconds / 60)} мин`
    : "Нет";
  return (
    <Card>
      <h3>SLA</h3>
          ...
          <dd>
            {ticket.is_response_breached || ticket.is_resolution_breached
              ...
              : ticket.status === "waiting_requester"
                ? "На паузе: ожидается ответ заявителя"
                : "В срок"}
          </dd>
        </div>
        <div>
...
export function ServiceDeskTicketDetailsPage() {
  ...
  const ticketQuery = useQuery({
    queryKey: serviceDeskQueryKeys.ticket(ticketId ?? "missing"),
    queryFn: ({ signal }) => getServiceDeskTicket(ticketId as string, signal),
  ...
  const setTicket = useCallback(
    (nextTicket: ServiceDeskTicket) => {
      if (ticketId) {
        queryClient.setQueryData(
          serviceDeskQueryKeys.ticket(ticketId),
          nextTicket,
        );
      }
    },
  ...
  const fieldEntries = ticket?.field_snapshot?.length
    ? ticket.field_snapshot
    : ticket
      ? Object.entries(ticket.field_values).map(([key, value]) => ({
          ...
          type: "text",
          raw_value: value,
          display_value: String(value ?? "Не указано"),
        }))
      : [];
  ...
  return (
    <>
      ...
      <PageLayout
        ...
        subtitle={
          ticket
            ? `${ticket.service.category.title} · ${ticket.service.title}`
            : undefined
        }
      >
        <nav className="breadcrumbs" aria-label="Навигация по заявкам">
          <Link to="/service-desk">Каталог</Link>
          <span>→</span>
          <Link to="/service-desk/my-tickets">Мои заявки</Link>
          <span>→</span>
          <span>{ticket?.number ?? "Заявка"}</span>
        </nav>
        {error && <p className="form-error">{error}</p>}
        {ticketQuery.isLoading && <Spinner label="Загружаем заявку" />}
        {!ticketQuery.isLoading && ticket && user && (
          <div className="service-desk-ticket-layout">
            <section className="service-desk-ticket-main">
              ...
              {ticket ? (
                <ServiceDeskTicketFieldAttachments
                  ticketId={ticket.id}
                  fields={fieldEntries.map((field) => ({
                    key: field.key,
                    label: field.label,
                    type: field.type,
                  }))}
                  ...
                  {ticket.history.map((event) => (
                    <li key={event.id}>
                      ...
                      <div>
                        ...
                        {historyDetails(event.payload) ? (
                          <p>{historyDetails(event.payload)}</p>
                        ) : null}
                      </div>
                    </li>
                  ))}
