...
describe("workbenchFilters", () => {
  it("updates page without resetting it to the first page", () => {
    ...
    expect(
      updateWorkbenchPage({ ...initialWorkbenchFilters, page: "2" }, 1, 3).page,
    ).toBe("1");
  });
  ...
  it("resets page when a filter changes", () => {
    const next = updateWorkbenchFilter(
      { ...initialWorkbenchFilters, page: "2" },
      "status",
      "assigned",
    );
  ...
  it("builds backend pagination and search params", () => {
    const params = buildWorkbenchParams(
      { ...initialWorkbenchFilters, page: "2", status: "assigned" },
      "SD-1",
    );
    expect(params.toString()).toBe(
      "page=2&page_size=25&status=assigned&q=SD-1",
    );
  });
  ...
  it("restores a valid quick view from the URL", () => {
    expect(
      getWorkbenchFiltersFromSearch(
        new URLSearchParams("quick_view=assigned_to_me"),
      ),
    ).toMatchObject({
      page: "1",
      page_size: "25",
      quick_view: "assigned_to_me",
    });
    expect(
      getWorkbenchFiltersFromSearch(new URLSearchParams("quick_view=unknown")),
    ).toEqual(initialWorkbenchFilters);
  });
