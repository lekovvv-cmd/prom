...
import type {
  ...
  WorkbenchTicket,
  WorkbenchUserOption,
} from "../../../entities/service-desk-workbench/model/types";
import type {
...
const payloadFields: Partial<
  Record<ServiceDeskAllowedAction, [string, string]>
> = {
...
export function ServiceDeskWorkbenchPage() {
  ...
  const quickViewFromSearch =
    getWorkbenchFiltersFromSearch(searchParams).quick_view ?? "";
  const [filters, setFilters] = useState(() =>
    getWorkbenchFiltersFromSearch(searchParams),
  );
  ...
  const [action, setAction] = useState<{
    ...
    action: ServiceDeskAllowedAction;
  } | null>(null);
  ...
  const params = useMemo(
    () => buildWorkbenchParams(filters, search),
    [filters, search],
  );
      ...
      try {
        ...
        setError(
          reason instanceof Error
            ? reason.message
            : "Не удалось загрузить рабочее место",
        );
      } finally {
  ...
  useEffect(
    () => subscribeToServiceDeskRefresh(() => void load({ silent: true })),
    [load],
  );

  useEffect(() => {
    setFilters((current) =>
      current.quick_view === quickViewFromSearch
        ? current
  ...
  function setPage(nextPage: number) {
    setFilters((current) =>
      updateWorkbenchPage(current, nextPage, data?.pages ?? 1),
    );
  }
  ...
  async function submitAction() {
    ...
    if ((field || action.action === "change_priority") && !payload.trim()) {
      setError(
        field
          ? `Заполните поле «${field[1]}»`
          : "Укажите причину изменения приоритета",
      );
      return;
    }
    setPending(true);
    try {
      ...
      if (action.action === "change_priority") {
        ...
        body.reason = payload.trim();
      }
      if (action.action === "assign" || action.action === "reassign")
        body.assignee_user_id = payload;
      await performWorkbenchAction(
        action.ticket.ticket_id,
        action.action,
        body,
        action.ticket.active_approval_id,
      );
      setAction(null);
      setPayload("");
  ...
  const hasFilters =
    Object.entries(filters).some(
      ([key, value]) => !["page", "page_size"].includes(key) && value,
    ) || Boolean(search);
  const showInitialSpinner = shouldShowInitialWorkbenchSpinner(
    loading,
    Boolean(data),
  );
  ...
  return (
    <>
      <PageLayout
        title="Рабочее место Service Desk"
        subtitle="Операционная очередь заявок, согласований и SLA"
      >
        <div className="workbench-quick-views">
          {counters &&
            (Object.keys(quickLabels) as WorkbenchQuickView[])
              ...
              .map((key) => (
                <Button
                  ...
                  onClick={() =>
                    updateQuickView(filters.quick_view === key ? "" : key)
                  }
                >
        ...
        <Card>
          <div className="filters workbench-filters">
            <Input
              ...
              placeholder="Номер, название или услуга"
            />
            <Select
              label="Статус"
            ...
            >
              {workbenchStatusOptions.map(([value, label]) => (
            ...
            <Select
              ...
              onChange={(event) =>
                updateFilter("assignee_user_id", event.target.value)
              }
            >
            ...
            <Select
              label="Заявитель"
              value={filters.requester_user_id ?? ""}
              onChange={(event) =>
                updateFilter("requester_user_id", event.target.value)
              }
            >
              <option value="">Все</option>
              {users.map((u) => (
            ...
            <Select
              ...
              onChange={(event) =>
                updateFilter("service_id", event.target.value)
              }
            >
              <option value="">Все</option>
              {services
                .filter(
                  (s) =>
                    !filters.category_id ||
                    s.category_id === filters.category_id,
                )
                .map((s) => (
            ...
            <Select
              ...
              onChange={(event) =>
                updateFilter("sla_state", event.target.value)
              }
            >
              <option value="">Все</option>
              <option value="no_sla">Без SLA</option>
            ...
            <Button
              variant="ghost"
              onClick={() => {
                setFilters(initialWorkbenchFilters);
                setSearch("");
              }}
            >
              Сбросить
        ...
        {showInitialSpinner ? (
          ...
          <>
            <ServiceDeskWorkbenchTable
              items={data.items}
              onAction={(ticket, selected) => {
                setAction({ ticket, action: selected });
                setPayload("");
            ...
            <div className="workbench-pagination">
              ...
              <span>
                Страница {filters.page} из {data.pages || 1} · {data.total}{" "}
                заявок
              </span>
              <Button
                variant="secondary"
                disabled={Number(filters.page) >= data.pages}
                onClick={() => setPage(Number(filters.page) + 1)}
              >
                Далее
              </Button>
          ...
          <EmptyState
            title={
              hasFilters
                ...
                : "В рабочем месте пока нет заявок"
            }
            text={
              hasFilters
                ? "Измените или сбросьте фильтры."
                : "Новые операционные заявки появятся здесь."
            }
          />
        )}
      </PageLayout>
        ...
        <Modal title="Действие с заявкой" onClose={() => setAction(null)}>
          <div className="modal-body">
            {action.action === "change_priority" ? (
              ...
              <label className="field">
                <span>{payloadFields[action.action]?.[1]}</span>
                <textarea
                  value={payload}
                  onChange={(event) => setPayload(event.target.value)}
                />
