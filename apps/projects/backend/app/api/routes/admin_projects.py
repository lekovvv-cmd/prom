...
from app.modules.projects.membership_service import ProjectMembershipService
...
def list_admin_projects(
    ...
    return ProjectQueryService(db).list_admin(
        search=search,
        status=status,
...
def create_admin_project(
    ...
    return ProjectCommandService(db).create(
        payload,
        actor=current_user,
...
def get_admin_project(project_id: UUID, current_user: AdminUser, db: DbSession) -> ProjectDetails:
    return ProjectQueryService(db).get_admin_details(project_id, current_user=current_user)
...
def update_admin_project(
    ...
    return ProjectCommandService(db).update(
        project_id,
        payload,
        current_user=current_user,
...
def archive_admin_project(
    ...
    ProjectCommandService(db).archive(
        project_id,
        current_user=current_user,
        expected_version=parse_if_match(if_match),
    )
...
def restore_admin_project(
    ...
    return ProjectCommandService(db).restore(
        project_id,
        current_user=current_user,
        expected_version=parse_if_match(if_match),
    )
...
def list_project_candidates(
    ...
    return ProjectRecommendationService(db).list_candidates(
...
def add_project_member(
    ...
    return ProjectMembershipService(db).add_working_group_member(
        project_id=project_id,
        user_id=user_id,
        current_user=current_user,
        idempotency_key=idempotency_key,
...
@router.delete("/{project_id}/members/{user_id}", response_model=ProjectDetails)
def remove_project_member(
    ...
    return ProjectMembershipService(db).remove_working_group_member(
        project_id=project_id,
        user_id=user_id,
        current_user=current_user,
        idempotency_key=idempotency_key,
