...
export function getAdminResponses(params?: ResponseListParams) {
  return apiClient.request<Paginated<ProjectResponse>>(
    `/admin/responses${toQuery(params)}`,
  );
}

export function getAdminProjectResponses(
  projectId: string,
  params?: Omit<ResponseListParams, "project_id" | "search">,
) {
  return apiClient.request<Paginated<ProjectResponse>>(
...
export function getMyResponses(
  params?: Pick<ResponseListParams, "limit" | "offset">,
) {
  return apiClient.request<Paginated<ProjectResponse>>(
    `/me/responses${toQuery(params)}`,
  );
}

export function updateAdminResponseStatus(
  responseId: string,
  status: ProjectResponseStatus,
) {
...
export function uploadProjectResponseAttachment(
  projectId: string,
  responseId: string,
  file: File,
) {
  ...
  return apiClient.request<Attachment>(
    `/projects/${projectId}/responses/${responseId}/attachments`,
    {
      method: "POST",
      body: formData,
    },
