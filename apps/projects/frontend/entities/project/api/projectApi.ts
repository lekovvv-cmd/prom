...
export function getProjects(params?: ProjectListParams, signal?: AbortSignal) {
  return apiClient.request<Paginated<Project>>(`/projects${toQuery(params)}`, {
    auth: false,
    signal,
  });
}
...
export function getProject(projectId: string, signal?: AbortSignal) {
  return apiClient.request<ProjectDetails>(`/projects/${projectId}`, {
    auth: false,
    signal,
  });
}

export function getProjectRecommendations(limit = 5, signal?: AbortSignal) {
  return apiClient.request<ProjectRecommendation[]>(
    `/projects/recommendations?limit=${limit}`,
    { signal },
  );
}

export function getAdminProjects(params?: ProjectListParams) {
  return apiClient.request<Paginated<Project>>(
    `/admin/projects${toQuery(params)}`,
  );
}
...
export function restoreArchivedAdminProject(projectId: string) {
  return apiClient.request<ProjectDetails>(
...
export function getAdminProjectCandidates(
  projectId: string,
  params?: ProjectCandidateParams,
) {
  return apiClient.request<Paginated<ProjectCandidate>>(
    `/admin/projects/${projectId}/candidates${toQuery(params)}`,
  );
}
...
export function uploadAdminProjectAttachment(projectId: string, file: File) {
  ...
  return apiClient.request<Attachment>(
    `/admin/projects/${projectId}/attachments`,
    {
      method: "POST",
      body: formData,
    },
