...
export function ProjectResponseForm({
  ...
  if (isLoading) {
    return (
      <section className="form-panel" id="response-form" aria-live="polite">
        ...
        <p className="muted">
          Форма отклика будет доступна после проверки пользователя.
        </p>
      </section>
    );
  }
  ...
  if (!user) {
    return (
      <section className="form-panel" id="response-form" aria-live="polite">
        ...
        <p className="muted">
          Отклик можно отправить только от авторизованного пользователя ТюмГУ.
        </p>
  ...
  if (user?.role === "platform_admin") {
    return (
      <section className="form-panel" id="response-form" aria-live="polite">
        ...
        <p className="muted">
          Администратор обрабатывает отклики и не может отправлять заявки на
          проекты.
        </p>
      </section>
    );
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(null);
      await submitProjectResponseWithFiles(
        projectId,
        {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось отправить отклик",
      );
    } finally {
  ...
  return (
    <form className="form-panel" id="response-form" onSubmit={handleSubmit}>
      ...
      />
      <div
        className="profile-competencies-readonly"
      ...
      <FileInput
        files={files}
        label="Прикрепить файлы к отклику"
        onChange={setFiles}
      />
      {error && <p className="form-error">{error}</p>}
      {success && (
        <div
          className="form-success form-success-card"
          role="status"
          aria-live="polite"
        >
          <FileCheck2 size={22} />
          <div>
            ...
            <span>
              Заявка уже в очереди обработки. Администратор увидит её в панели
              откликов.
            </span>
          </div>
        </div>
