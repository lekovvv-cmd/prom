...
function projectToForm(project: ProjectDetails): ProjectMutationPayload {
  return {
    ...
    competency_blocks: normalizeCompetencyBlocks(
...
export function EditProjectForm({
  ...
  const [form, setForm] = useState<ProjectMutationPayload>(
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(null);
      const updatedProject = await editProjectWithFiles(
        project.id,
  ...
  return (
    <form className="form-panel" onSubmit={handleSubmit}>
      ...
      <FileInput
        files={files}
        label="Добавить файлы к проекту"
        onChange={setFiles}
      />
