...
import { formatDate, formatDateTime } from "@prom/utils/date";
...
export function getProjectManagementMetrics(
  project: ProjectDetails,
  responses: ProjectResponse[],
) {
  const workingGroupSize = project.members.filter(
    (member) => member.member_role === "working_group_member",
  ).length;
  const newResponses = responses.filter(
    (response) => response.status === "new",
  ).length;
...
export function AdminProjectManagePage() {
  ...
  const plannedTasks = useMemo(
    () => splitProjectTasks(project?.planned_tasks),
      ...
      >
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            text="Проверьте ссылку или права доступа к проекту."
          />
        ) : (
          <div className="project-management-page">
            <section
              className="project-management-hero"
            ...
            {metrics && (
              <section
                className="project-management-metrics"
                aria-label="Показатели проекта"
              >
              ...
              <section className="project-management-main">
                  ...
                  <Card>
                    ...
                    {competencyBlocks.length > 0 && (
                      <div className="details-section">
                        ...
                        <CompetencyCoverage
                          blocks={competencyBlocks}
                          coverage={project.competency_coverage}
                        />
                      </div>
                    )}
                ...
                <Card className="project-responses-section">
                  <div className="section-heading">
                    ...
                    <span>
                      {responses.length} из {project.responses_count}
                    </span>
                  </div>
                  <AdminResponsesTable
                    responses={responses}
