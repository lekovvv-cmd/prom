...
import { SearchableSelect } from "@prom/ui/SearchableSelect";
...
export function AdminResponsesPage() {
  ...
  async function loadResponses() {
    try {
      setIsLoading(true);
      setError(null);
      const responsesPayload = await getAdminResponses({
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось загрузить отклики",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function loadProjects(query: string) {
    try {
      ...
      const [currentProjectsPayload, archivedProjectsPayload] =
        await Promise.all([
          getAdminProjects({ search: query, limit: 100 }),
          getAdminProjects({ search: query, status: "archived", limit: 100 }),
        ]);
      const uniqueProjects = [
        ...currentProjectsPayload.items,
        ...archivedProjectsPayload.items,
      ].filter(
        (project, index, items) =>
          items.findIndex((item) => item.id === project.id) === index,
      );
  ...
  const projectOptions = useMemo(() => {
    ...
    if (
      selectedProject &&
      !options.some((option) => option.value === selectedProject.id)
    ) {
      options.unshift({
  ...
  return (
    <>
      <Header />
      <PageLayout title="Очередь откликов">
        <div className="filters">
          ...
          <Select
            ...
            onClear={() => setStatus("")}
            onChange={(event) =>
              setStatus(event.target.value as ProjectResponseStatus | "")
            }
          >
