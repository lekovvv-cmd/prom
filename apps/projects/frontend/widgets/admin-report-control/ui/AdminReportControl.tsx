...
function normalizePeriodPayload(
  form: ReportPeriodPayload,
): ReportPeriodPayload {
  return {
...
export function AdminReportControl() {
  ...
  useEffect(() => {
    ...
    reload()
      .catch((err) => {
        if (isMounted) {
          setError(
            err instanceof Error ? err.message : "Не удалось загрузить отчёты",
          );
        }
      })
      .finally(() => {
  ...
  async function handleOpenPeriod(event: FormEvent) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось открыть период",
      );
    } finally {
  ...
  async function handleClosePeriod(periodId: string) {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось закрыть период",
      );
    } finally {
      setIsSubmitting(false);
    }
    ...
    <Card className="report-card admin-report-card">
      <div className="section-heading">
        <div>
          ...
          <p>
            Администратор открывает период вручную. Сотрудники и руководители
            подают отчёты в профиле.
          </p>
        </div>
        {activePeriod ? (
      ...
      <div className="report-submissions">
        ...
        {reports.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              Они появятся после отправки формы сотрудником или руководителем.
            </p>
          </div>
        ) : (
