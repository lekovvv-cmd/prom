...
export function ProjectTasksPanel({
  ...
  async function uploadTaskFiles(task: ProjectTask) {
    ...
    try {
      ...
      setError(null);
      await Promise.all(
        files.map((file) => uploadProjectTaskAttachment(task.id, file)),
      );
      setFilesByTask((current) => ({ ...current, [task.id]: [] }));
      await loadTasks();
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Не удалось прикрепить результат",
      );
    } finally {
      setBusyTaskId(null);
    }
    ...
    <Card className="project-tasks-panel">
        ...
        <div className="project-task-forms">
          <form className="task-stage-form" onSubmit={handleCreateStage}>
            <Input
              ...
              onChange={(event) =>
                setStageForm({ ...stageForm, title: event.target.value })
              }
            ...
            <Input
              ...
              onChange={(event) =>
                setStageForm({ ...stageForm, start_date: event.target.value })
              }
            />
            <Input
              label="Финиш"
              ...
              onChange={(event) =>
                setStageForm({ ...stageForm, end_date: event.target.value })
              }
            />
          ...
          <form className="task-create-form" onSubmit={handleCreateTask}>
            ...
            <Select
              ...
              onChange={(event) =>
                setTaskForm({ ...taskForm, stage_id: event.target.value })
              }
            >
      ...
      {isLoading ? (
        ...
        <TaskList
          ...
          mode={mode}
          onFilesChange={(taskId, files) =>
            setFilesByTask((current) => ({ ...current, [taskId]: files }))
          }
          onStatusChange={updateTaskStatus}
          onUpload={uploadTaskFiles}
          tasks={assignedTasks}
...
function TaskList({
  ...
  return (
    <div className="task-list-grid">
      {tasks.map((task) => (
        <article
          className={`task-card ${task.is_overdue ? "task-card-overdue" : ""}`}
          key={task.id}
        >
          ...
          <dl className="task-meta">
            ...
            <div>
              <dt>Дедлайн</dt>
              <dd className={task.is_overdue ? "task-overdue-text" : ""}>
          ...
          <Select
            ...
            onChange={(event) =>
              onStatusChange(task, event.target.value as ProjectTaskStatus)
            }
          >
