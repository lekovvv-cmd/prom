...
export function ApproveTicketButton({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Не удалось согласовать заявку");
    } finally {
      ...
      {isOpen && (
        <Modal title="Согласовать заявку" onClose={() => setIsOpen(false)}>
          <form className="form-panel" onSubmit={handleSubmit}>
            <p className="muted">Комментарий необязателен и будет сохранён в истории решения.</p>
            <Textarea
              label="Комментарий"
              value={comment}
              maxLength={2000}
