...
export async function submitProjectResponseWithFiles(projectId: string, payload: ProjectResponsePayload, files: File[]) {
  const response = await submitProjectResponse(projectId, payload);
  await Promise.all(files.map((file) => uploadProjectResponseAttachment(projectId, response.id, file)));
  return response;
}
