...
export function ServiceDeskTicketApprovals({
  ...
  const { canApprove, canReject } = getTicketActionAvailability(ticket.allowed_actions);
  ...
  return (
    <Card className="service-desk-approvals">
      <div className="service-desk-section-heading">
        ...
        <Badge tone={ticket.status === "pending_approval" ? "warning" : "neutral"}>
      ...
      {ticket.approval_stages.length === 0 ? (
        ...
        <ol className="approval-stage-list">
          {ticket.approval_stages.map((stage) => {
            const isCurrent = stage.status === "pending" && Boolean(stage.started_at);
            ...
            return (
              <li
                ...
                key={stage.id}
              >
                <div className="approval-stage-marker">{stage.position + 1}</div>
                <div className="approval-stage-content">
                  <div className="approval-stage-header">
                    <div>
                      <h4>{stage.title}</h4>
                      <span>
                        Правило: {stage.decision_rule === "any" ? "достаточно одного" : "нужны все"}
                      </span>
                    </div>
                  ...
                  <ul className="approval-person-list">
                    {stage.approvals.map((approval) => {
                      ...
                      return (
                        <li key={approval.id}>
                          <span className={`approval-person-icon badge-${meta.tone}`}>
                            <StatusIcon size={14} />
                          </span>
                          <div>
                            ...
                            {approval.decision_comment && <p>{approval.decision_comment}</p>}
                          </div>
                        </li>
                      );
                    })}
