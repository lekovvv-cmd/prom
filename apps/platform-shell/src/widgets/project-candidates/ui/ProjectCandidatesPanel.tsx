...
export function ProjectCandidatesPanel({ project, onMemberAdded }: Props) {
  ...
  const competencyOptions = useMemo(
    () => Array.from(new Set(project.competency_blocks.flatMap((block) => block.competencies))),
  ...
  async function addMember(userId: string) {
    try {
      ...
      setError(err instanceof Error ? err.message : "Не удалось добавить сотрудника в проект");
    } finally {
    ...
    <Card className="project-candidates-section">
      <div className="section-heading">
        <div>
          ...
          <p className="muted">Поиск по профилям, компетенциям и направлениям работы проекта.</p>
        </div>
      ...
      <div className="candidate-filters">
        ...
        <Select
          ...
          onChange={(event) => updateFilter({ block_title: event.target.value })}
        >
        ...
        <Select
          ...
          onChange={(event) => updateFilter({ sort: event.target.value as ProjectCandidateParams["sort"] })}
        >
      ...
      {isLoading ? (
        ...
        <EmptyState title="Кандидаты не найдены" text="Измените фильтры или попросите сотрудников заполнить профиль." />
      ) : (
  ...
  return (
    <article className="candidate-row">
      <div className="candidate-main">
        ...
        <p>
          {[candidate.position, candidate.department].filter(Boolean).join(" - ") || "Профиль без должности"}
        </p>
        ...
        <div className="competency-inline">
          {competencies.slice(0, 8).map((competency) => (
            <span
              className={
                candidate.matched_competencies.includes(competency) ? "chip chip-selected" : "chip"
              }
              key={competency}
            >
              {competency}
      ...
      <div className="candidate-side">
        ...
        {candidate.matched_blocks.length > 0 && <span>{candidate.matched_blocks.join(", ")}</span>}
        {candidate.has_response && <span>Уже откликался</span>}
        <Button type="button" variant={canAdd ? "secondary" : "ghost"} disabled={!canAdd || isProcessing} onClick={onAdd}>
          {candidate.is_working_group_member ? "В группе" : isProcessing ? "Добавляем" : "Добавить"}
        </Button>
      </div>
    </article>
  );
