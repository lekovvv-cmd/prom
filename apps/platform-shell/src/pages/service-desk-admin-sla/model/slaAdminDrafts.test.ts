...
describe("SLA admin payload builders", () => {
  it("keeps arbitrary business hours and multiple custom-hour intervals", () => {
    const payload = calendarDraftToPayload({
      ...
      exceptions: [
        ...
        { date: "2026-08-02", type: "custom_hours", start_time: "09:00", end_time: "12:00" },
        { date: "2026-08-02", type: "custom_hours", start_time: "13:00", end_time: "15:00" }
      ]
    });
  ...
  it("includes specific recipient and clears it for other recipient types", () => {
    ...
    expect(escalationDraftToPayload(base).recipient_user_id).toBe("recipient-id");
    expect(
      escalationDraftToPayload({ ...base, recipientType: "requester" }).recipient_user_id
    ).toBeNull();
  });
});
