...
export const BINDING_FIELD_OPTIONS: Array<{ value: SlaBindingConditionField; label: string }> = [
...
export function calendarToDraft(calendar: BusinessCalendar): CalendarDraft {
  return {
    ...
    businessHours: calendar.business_hours.map(({ weekday, start_time, end_time }) => ({
      weekday,
      start_time: start_time.slice(0, 5),
      end_time: end_time.slice(0, 5)
    })),
    exceptions: calendar.exceptions.map(({ date, type, start_time, end_time, description }) => ({
      date,
      type,
      start_time: start_time?.slice(0, 5),
      end_time: end_time?.slice(0, 5),
...
export function calendarDraftToPayload(draft: CalendarDraft): BusinessCalendarPayload {
  return {
...
export function bindingDraftToPayload(draft: BindingDraft): SlaBindingPayload {
  return {
    ...
    conditions: draft.conditions.map((condition) => ({
      ...
      field_key: condition.field === "field_value" ? condition.field_key?.trim() : undefined
    }))
  };
}
...
export function escalationDraftToPayload(draft: EscalationDraft): EscalationRulePayload {
  return {
    metric: draft.metric,
    threshold_percent: Number(draft.thresholdPercent),
    action_type: draft.actionType,
    recipient_type: draft.recipientType,
    recipient_user_id: draft.recipientType === "specific_user" ? draft.recipientUserId : null,
    is_active: draft.isActive
  };
}
