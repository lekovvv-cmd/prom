...
export function getServiceDeskRoutingRules() {
  return serviceDeskApiClient.request<ServiceDeskRoutingRule[]>("/admin/routing-rules");
}
...
export function updateServiceDeskRoutingRule(
  ...
  return serviceDeskApiClient.request<ServiceDeskRoutingRule>(`/admin/routing-rules/${ruleId}`, {
    method: "PATCH",
    body: JSON.stringify(payload)
  });
}
...
export function reorderServiceDeskRoutingRules(ruleIds: string[]) {
  return serviceDeskApiClient.request<ServiceDeskRoutingRule[]>("/admin/routing-rules/reorder", {
    method: "POST",
