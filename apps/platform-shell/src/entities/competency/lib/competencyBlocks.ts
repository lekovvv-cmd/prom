...
export const DEFAULT_COMPETENCY_BLOCK_TITLE = "Общие компетенции";
...
export function blocksFromCompetencyString(value: string | null | undefined): ProjectCompetencyBlock[] {
...
export function normalizeCompetencyBlocks(
  ...
  const normalized = (blocks ?? [])
    .map((block) => ({
      ...
      competencies: splitCompetencies(joinCompetencies(block.competencies ?? []))
    }))
    .filter((block) => block.competencies.length > 0);

  return normalized.length > 0 ? normalized : blocksFromCompetencyString(fallbackCompetencies);
}

export function flattenCompetencyBlocks(blocks: ProjectCompetencyBlock[]): string {
...
export function getCoverageForBlock(
  ...
  const coverageByName = new Map(coverage.map((item) => [item.competency.toLocaleLowerCase("ru-RU"), item]));
