...
export function isFieldVisible(field: { visibility_rules?: FormRules }, values: FormValues) {
  return matchesRules(field.visibility_rules, values, true);
}

export function isFieldRequired(field: { is_required: boolean; required_rules?: FormRules }, values: FormValues) {
...
export function normalizeFieldValue(fieldType: string, value: unknown) {
  ...
  if (fieldType === "multiselect") return Array.isArray(value) ? value.map(String) : [];
  return value;
}
