...
test("загрузка услуг в согласованиях показывает понятную ошибку и повторяется", async ({
  page,
}) => {
  await loginAs(
    page,
  ...
  await page.route("**/api/service-desk/v1/admin/services", async (route) => {
  ...
  await page.unroute("**/api/service-desk/v1/admin/services");
  await page.getByRole("button", { name: "Повторить" }).click();
  await expect(page.getByRole("alert")).toHaveCount(0);
  await expect
    .poll(async () => page.getByLabel("Услуга").locator("option").count())
    .toBeGreaterThan(1);
});

test("администратор включает, изменяет и отключает согласование у услуги", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Согласования" }),
  ).toBeVisible();
  await page.getByLabel("Услуга").selectOption({ label: serviceTitle });
  await expect(page.getByText("Опубликованная версия 1")).toBeVisible();
  await expect(
    page.getByText("Изменения применятся только к новым заявкам."),
  ).toBeVisible();
  ...
  await page
    .getByLabel("Добавить согласующего к этапу 1")
    .selectOption({ index: 1 });
  await page.getByRole("button", { name: "Добавить согласующего" }).click();
  await expect(page.locator(".approval-approver-tag")).toHaveCount(1);
  await page.getByRole("button", { name: "Сохранить и применить" }).click();
  await expect(
    page.getByText(
      "Версия 2 опубликована. Предыдущая версия перенесена в архив.",
    ),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "Версия 3 опубликована. Предыдущая версия перенесена в архив.",
    ),
  ).toBeVisible();
  await expect(page.getByLabel("Название этапа 1")).toHaveValue(
    editedStageTitle,
  );

  await page.getByRole("button", { name: "Отключить согласование" }).click();
  await expect(
    page.getByText("Новые заявки будут создаваться без согласования."),
  ).toBeVisible();
  await page.getByRole("button", { name: "Сохранить и применить" }).click();
  await expect(
    page.getByText(
      "Версия 4 опубликована. Предыдущая версия перенесена в архив.",
    ),
  ).toBeVisible();
