    ...
    async cleanup(page: Page) {
      ...
      const failures = await page.evaluate(async (items) => {
        ...
        for (const item of items) {
          ...
          const deactivateByTitle = async (
            kind: "services" | "categories",
            title: string,
          ) => {
            const listed = await fetch(
              `/api/service-desk/v1/admin/${kind}?q=${encodeURIComponent(title)}`,
              { headers },
            );
            if (!listed.ok) {
              errors.push(
                `Не удалось найти ${kind} «${title}»: ${listed.status}`,
              );
              return;
            }
            ...
            const matching = entries.filter(
              (entry) => entry.title === title && entry.deleted_at === null,
            );
            for (const entry of matching) {
              const response = await fetch(
                `/api/service-desk/v1/admin/${kind}/${entry.id}/deactivate`,
                {
                  method: "POST",
                  headers,
                },
              ...
              if (!response.ok)
                errors.push(
                  `Не удалось отключить ${kind} «${title}»: ${response.status}`,
                );
            }
          };

          for (const title of item.serviceTitles ?? [])
            await deactivateByTitle("services", title);
          for (const title of item.categoryTitles ?? [])
            await deactivateByTitle("categories", title);
        }
      ...
      if (failures.length)
        throw new Error(
          `Не удалось очистить E2E-фикстуры каталога:\n${failures.join("\n")}`,
        );
    },
  };
}
