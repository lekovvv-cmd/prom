...
test("admin catalog CRUD validates duplicate category titles", async ({
  page,
}, testInfo) => {
  ...
  await categoryRow
    .getByLabel("Описание")
    .fill("Создано browser QA через production UI");
  ...
  await expect(
    services.locator(".admin-config-row").filter({ hasText: serviceTitle }),
  ).toBeVisible();
  ...
  await categories
    .getByLabel("Новая категория")
    .fill(editedTitle.toUpperCase());
  await categories.getByRole("button", { name: "Создать" }).click();
  await expect(page.getByRole("alert")).toContainText("уже существует");
  await expect(
    categories.locator(".admin-config-row").filter({ hasText: editedTitle }),
  ).toHaveCount(1);
