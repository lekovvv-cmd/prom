...
test("access management creates, edits capabilities and deactivates by Projects user", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Менеджеры и права" }),
  ).toBeVisible();
  ...
  if ((await row.count()) === 0) {
    ...
    const projectUserId = await projectUser
      .locator("option")
      .filter({ hasText: "employee@utmn.ru" })
      .getAttribute("value");
    ...
    await page
      .getByRole("dialog", { name: "Активировать доступ" })
