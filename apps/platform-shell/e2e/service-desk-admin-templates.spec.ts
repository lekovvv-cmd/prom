...
test("template preview keeps controls aligned within each row", async ({
  page,
}, testInfo) => {
  ...
  await page
    .getByLabel("Услуга")
    .selectOption({ label: "Транспортное обслуживание" });
...
test("admin creates and publishes a template with production preview", async ({
  page,
}, testInfo) => {
  const diagnostics = watchPage(page);
  ...
  await editor
    .getByLabel("Название поля для пользователя")
    .fill(dependentLabel);
  await expect(
    editor.getByText(`extra_details_${suffix}`, { exact: true }),
  ).toHaveCount(0);
  await editor
    .getByText("Показывать это поле только при выполнении условия", {
      exact: true,
    })
    .click();
  await editor
    .getByLabel("Поле для условия видимости 1")
    .selectOption(`contact_email_${suffix}`);
  await editor.getByLabel("Значение для условия 1").fill("yes");
  await expect(
    editor.getByText("Поле не может ссылаться само на себя."),
  ).toHaveCount(0);
  await editor.getByRole("button", { name: "Сохранить поле" }).click();
  await expect(page.getByText(dependentLabel)).toBeVisible();
  await page.getByRole("button", { name: "Открыть предпросмотр" }).click();
  await expect(
    page.getByRole("heading", { name: "Предпросмотр формы" }),
  ).toBeVisible();
  await expect(
    page.getByRole("textbox", { name: `${fieldLabel} *` }),
  ).toBeVisible();
  ...
  const publishedVersion = page
    .getByLabel("Версии формы")
    .getByRole("button", { name: "Версия 1" });
