...
async function login(page: Page, role: string, next = "/projects") {
  ...
  await page
    .getByRole("button", { name: new RegExp(role), exact: true })
    .click();
...
for (const role of ["Сотрудник", "Руководитель проектов"]) {
  test(`${role}: Service Desk скрыт и прямой URL закрыт`, async ({ page }) => {
    ...
    await expect(
      page.getByLabel("Переключатель модулей").getByText("Service Desk"),
    ).toHaveCount(0);
...
test("Service Desk manager: каталог и заявки доступны без admin sections", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("banner").getByRole("link", { name: "Мои заявки" }),
  ).toBeVisible();
  await expect(
...
test("Service Desk admin: полный admin navigation доступен", async ({
  page,
}) => {
  await login(page, "Администратор Service Desk", "/admin/service-desk");
  await expect(
    page.getByRole("heading", { name: "Service Desk - обзор" }),
  ).toBeVisible();
  ...
  for (const item of [
    ...
    await expect(
      adminNavigation.getByRole("link", { name: item, exact: true }),
    ).toBeVisible();
  }
});

test("platform admin: Service Desk работает без заранее созданного local profile", async ({
  page,
}) => {
...
test("пустая форма показывает errors и фокусирует первое поле", async ({
  page,
}) => {
  await login(page, "Менеджер Service Desk", "/service-desk");
