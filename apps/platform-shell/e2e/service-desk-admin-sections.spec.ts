...
test("dictionaries support create, values, duplicate validation and activation through UI", async ({
  page,
}, testInfo) => {
  ...
  await page.unroute("**/api/service-desk/v1/admin/dictionaries");
  await page.getByRole("button", { name: "Повторить" }).click();
  await expect(page.getByRole("alert")).toHaveCount(0);
  await expect(page.locator("body")).not.toContainText(
    /системн|служебн|внутренн/i,
  );
  await expect(
    page.getByRole("button", { name: "Создать справочник" }),
  ).toBeDisabled();
  ...
  const itemRow = page
    .locator(".admin-config-row")
    .filter({ hasText: "Первое значение" });
  ...
  await expect(
    page.getByRole("button", { name: "Включить справочник" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Включить справочник" }).click();
  await page.reload();
  await page.getByRole("button", { name: title }).click();
  await expect(
    page.getByText("Первое значение", { exact: true }),
  ).toBeVisible();
  await attachScreenshot(page, testInfo, "dictionary-admin");
  diagnostics.assertClean(
    (response) =>
      response.startsWith("409 POST") &&
      response.includes("/admin/dictionaries/"),
    (message) =>
...
test("routing rule supports validation, create, edit, reload and delete through UI", async ({
  page,
}) => {
  const diagnostics = watchPage(page);
  const suffix = Date.now();
  ...
  await expect(
    page.getByRole("heading", { name: "Маршрутизация Service Desk" }),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Сохранить правило" }).click();
  let card = page
    .locator(".service-desk-routing-rule-card")
    .filter({ hasText: name });
  ...
  diagnostics.assertClean(undefined, (message) =>
    message.includes("Failed to load resource: net::ERR_FAILED"),
  );
});

test("remaining admin sections satisfy direct-link page contract on desktop and mobile", async ({
  page,
}, testInfo) => {
  const diagnostics = watchPage(page);
