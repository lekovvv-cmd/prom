...
test("selector keeps both anonymous module choices and next redirects", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("link", { name: /Проектный модуль/ }),
  ).toHaveAttribute("href", "/login?next=%2Fprojects");
  const serviceDeskCard = page
    .locator(".module-selector-card")
    .filter({ hasText: "Service Desk" });
  ...
  await expect(page).toHaveURL(/\/login\?next=%2Fservice-desk$/);
  await expect(
    page.getByRole("heading", { name: "Вход в PROM" }),
  ).toBeVisible();
  await expect
    .poll(async () =>
