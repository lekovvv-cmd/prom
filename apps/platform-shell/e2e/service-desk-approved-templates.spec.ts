...
const approvedTemplates: ApprovedTemplate[] = [
  {
    category: "ГИА: Администрирование",
    service: "Заказ воды",
    labels: [
      "Количество бутылок",
      "Объем бутылок",
      ...
      "Количество заседаний ГЭК",
      "Количество выпускающихся студентов",
    ],
  },
  {
    category: "ГИА: Администрирование",
    service: "Установка камер",
    labels: [
      "Институт",
      "Вид ГИА",
      "Направление (специальность)",
      "Номер аудитории для установки камер",
      "Дата и время начала мероприятия",
      "Дата и время окончания мероприятия",
      "комментарии",
    ],
  ...
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Бронирование аудиторий",
    labels: [
      "Цель брони аудитории",
      "ФИО (с кем согласовано)",
      "Адрес корпуса брони аудитории",
      "Номер аудитории",
      "Дата и время начала мероприятия",
      "Дата и время окончания мероприятия",
      "Комментарий",
  ...
  {
    category: "Административно-хозяйственное сопровождение",
    service: "На печать в Издательство",
    labels: [
      "Название мероприятия",
      "Согласовано/не согласовано с издательством",
      "Вид продукции",
      "Сроки изготовления",
  ...
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Роль табельщика (табель рабочего времени, график отпусков)",
    labels: ["ФИО", "Дата начала отпуска", "Дата окончания отпуска"],
  },
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Ввоз (вывоз) и внос (вынос) материальных ценностей",
    labels: [
      "Название мероприятия",
      "Дата и время вноса (ввоза)",
      "Дата и время выноса (вывоза)",
      "Прикрепите файл с перечнем МЦ",
      "Вид материальных ценностей",
      "Тара",
      "Номер, модель ТС при ввозе (вывозе)",
    ],
  },
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Допуск в здание",
    labels: [
      "ФИО (на кого оформляется допуск)",
      "Оборудование",
      "Адрес",
      "Дата начала доступа",
      "Дата прекращения доступа",
    ],
  },
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Транспортное обслуживание",
    labels: [
      "Название мероприятия",
  ...
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Оформление и регистрация исходящего документа",
    labels: ["Документ", "Проект или скан документа"],
  },
  {
    category: "Административно-хозяйственное сопровождение",
    service: "График отпусков",
    labels: ["ФИО", "Дата начала отпуска", "Дата окончания отпуска"],
  },
  {
    category: "Административно-хозяйственное сопровождение",
    service: "Получение со склада (кроме компьютерной техники)",
    labels: [
      "Материально ответственное лицо ФИО",
      "Должность",
      "Мероприятие",
      "Список для выдачи со склада",
    ],
  },
];
...
async function openTemplate(page: Page, template: ApprovedTemplate) {
  ...
  const serviceCard = categoryGroup
    .locator(".service-desk-service-card")
    .filter({
      has: page.getByRole("heading", { name: template.service, exact: true }),
    });
  const serviceHeading = serviceCard.getByRole("heading", {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: template.service }),
  ).toBeVisible();
}
...
test("all eleven approved templates render exact labels and reject an empty submission", async ({
  page,
}) => {
  ...
  for (const template of approvedTemplates) {
    ...
    for (const label of template.labels) {
      const field =
        label === "Документ"
          ? page.locator("#document")
          : fieldByLabel(page, label);
...
test("approved templates submit with required dictionaries and files", async ({
  page,
}) => {
  const diagnostics = watchPage(page);
  ...
  await page
    .getByLabel("Направление (специальность)")
    .selectOption("40_03_01_law");
  ...
  await page
    .getByLabel("Адрес корпуса брони аудитории")
    .selectOption("lenina_16");
  ...
  await page
    .getByLabel("Согласовано/не согласовано с издательством")
  ...
  await page
    .getByLabel("Дата и время выноса (вывоза)")
    .fill("2027-01-15T18:00");
  await page.locator("#inventory_list_file").setInputFiles({
    name: "inventory.txt",
    mimeType: "text/plain",
  ...
  await page
    .getByLabel("Номер, модель ТС при ввозе (вывозе)")
    .fill("А123АА, Газель");
  await fillCommon(page, `E2E МЦ ${suffix}`);
  await submit(page);
  await expect(
    page.getByText("Прикрепите файл с перечнем МЦ: inventory.txt", {
      exact: true,
    }),
  ).toBeVisible();
  ...
  await page.locator("#document_file").setInputFiles({
    name: "document.txt",
    mimeType: "text/plain",
  ...
  await submit(page);
  await expect(
    page.getByText("Проект или скан документа: document.txt", { exact: true }),
  ).toBeVisible();

  await openTemplate(page, approvedTemplates[10]);
  await page
    .getByLabel("Материально ответственное лицо ФИО")
    .fill("Петров П.П.");
  ...
  await page.locator("#inventory_list_file").setInputFiles({
    name: "warehouse.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("Warehouse"),
  });
  await fillCommon(page, `E2E склад ${suffix}`);
  await submit(page);
  await expect(
    page.getByText("Список для выдачи со склада: warehouse.txt", {
      exact: true,
    }),
  ).toBeVisible();
  await page.reload();
