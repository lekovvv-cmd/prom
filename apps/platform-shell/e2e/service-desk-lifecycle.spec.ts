...
test("my tickets direct URL, filters and reload keep a meaningful state", async ({
  page,
}) => {
  ...
  await page
    .getByRole("tablist", { name: "Фильтр заявок" })
    .getByRole("button", { name: "Черновики" })
    .click();
  await expect(page.getByRole("button", { name: "Черновики" })).toHaveClass(
    /active/,
  );
  await page.reload();
  await expect(page.getByRole("heading", { name: "Мои заявки" })).toBeVisible();
