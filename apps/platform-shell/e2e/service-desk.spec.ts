...
const serviceDeskUrl =
  process.env.E2E_SERVICE_DESK_URL ?? "http://127.0.0.1:8001";
...
async function loginAsManager(page: Page) {
  ...
  await page
    .getByRole("button", { name: /Администратор Service Desk/ })
    .click();
...
test("Service Desk flow: approver reviews and approves a ticket", async ({
  page,
  request,
}) => {
  const token = await loginAsManager(page);
  const currentUser = await serviceDeskRequest<{ id: string }>(
    request,
    token,
    "get",
    "/me",
  ...
  catalogFixtures.trackWithToken(token, {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: ticket.number }),
  ).toBeVisible();
  await expect(
    page.getByLabel("Счётчики Service Desk").getByText("Моё согласование"),
  ).toBeVisible();
  await expect(
    page.getByLabel("Счётчики Service Desk").getByText("Нарушения SLA"),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: `E2E заявка ${suffix}` }),
  ).toBeVisible();
  ...
  const approvalNotification = notificationCenter
    ...
    .filter({
      has: page.locator(`a[href="/service-desk/tickets/${ticket.id}"]`),
    });
  await expect(
    approvalNotification.getByText("Требуется согласование"),
  ).toBeVisible();
  await expect(
    approvalNotification.getByRole("link", { name: "Открыть заявку" }),
  ).toHaveAttribute("href", `/service-desk/tickets/${ticket.id}`);
  await notificationCenter
    .getByRole("button", { name: "Прочитать все" })
    .click();
  ...
  await dialog
    .getByRole("button", { name: "Подтвердить согласование" })
    .click();

  await expect(dialog).toBeHidden();
  await expect(page.getByText("Согласована")).toBeVisible();
  await expect(
    page.locator(".service-desk-approvals").getByText("E2E: согласовано"),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Согласовать" })).toHaveCount(
    0,
  );
  await expect(page.getByRole("button", { name: "Отклонить" })).toHaveCount(0);
});

test("Service Desk SLA admin persists complete calendar, policy, binding and escalation edits", async ({
  page,
  request,
}) => {
  const token = await loginAsManager(page);
  const suffix = Date.now();
  const recipients = await serviceDeskRequest<
    Array<{ id: string; email: string }>
  >(request, token, "get", "/admin/sla/recipients");
  const managerRecipient = recipients.find(
    (recipient) => recipient.email === "sd.manager@utmn.ru",
  );
  expect(managerRecipient).toBeTruthy();
  await page.goto("/service-desk/admin/sla");
  await expect(
    page.getByRole("heading", { name: "Настройка SLA" }),
  ).toBeVisible();
  ...
  const calendarSection = page.locator(
    'section[aria-labelledby="sla-calendars-title"]',
  );
  const calendarForm = calendarSection.locator("form");
  await calendarForm.getByLabel("Название календаря").fill(calendarName);
  ...
  await calendarForm
    .getByRole("button", { name: "Интервал", exact: true })
    .click();
  ...
  await calendarForm
    .locator(".service-desk-sla-advanced-settings summary")
    .click();
  await calendarForm
    .getByRole("button", { name: "Исключение", exact: true })
    .click();
  ...
  await calendarForm
    .getByLabel("Тип исключения 2")
    .selectOption("custom_hours");
  ...
  const calendarCard = calendarSection
    .locator(".service-desk-sla-summary-card")
  ...
  await calendarForm
    .getByLabel("Название календаря")
    .fill(`${calendarName} edited`);
  await calendarForm
    .getByRole("button", { name: "Сохранить изменения" })
    .click();
  await expect(
    calendarSection.getByText(`${calendarName} edited`),
  ).toBeVisible();
  ...
  await policyForm.getByLabel("Название политики").fill(policyName);
  await policyForm
    .locator(".service-desk-sla-advanced-settings summary")
    .click();
  await policyForm
    .getByLabel("Описание для команды")
    .fill("E2E critical policy");
  await policyForm
    .getByLabel("Рабочий календарь")
    .selectOption({ label: `${calendarName} edited` });
  await policyForm.getByLabel("Срок первого ответа, минут").fill("15");
  await policyForm.getByLabel("Срок решения, минут").fill("240");
  await policyForm
    .getByLabel("Останавливать срок, когда ждём заявителя")
    .check();
  await policyForm.getByRole("button", { name: "Создать" }).click();
  const policyCard = policySection
    .locator(".service-desk-sla-summary-card")
    .filter({ hasText: policyName });
  await policyCard.getByRole("button", { name: "Изменить" }).click();
  await policyForm.getByLabel("Срок решения, минут").fill("300");
  ...
  const bindingSection = page.locator(
    'section[aria-labelledby="sla-bindings-title"]',
  );
  const bindingForm = bindingSection.locator("form");
  await bindingForm.getByLabel("Название правила").fill(bindingName);
  await bindingForm
    .getByLabel("Применять политику")
  ...
  await bindingForm
    .getByRole("button", { name: "Ещё условие", exact: true })
    .click();
  ...
  const bindingCard = bindingSection
    .locator(".service-desk-sla-summary-card")
    .filter({ hasText: bindingName });
  await expect(bindingCard).toContainText("2 условий");
  await bindingCard.getByRole("button", { name: "Изменить" }).click();
  await bindingForm
    .locator(".service-desk-sla-advanced-settings summary")
    .click();
  await bindingForm.getByLabel("Порядок применения").fill("12");
  await bindingForm
    .getByRole("button", { name: "Сохранить изменения" })
    .click();
  await expect(bindingCard).toContainText("Порядок 12");

  const escalationSection = page.locator(
    'section[aria-labelledby="sla-escalations-title"]',
  );
  await escalationSection
    .locator(".service-desk-sla-advanced-settings summary")
    .click();
  const escalationForm = escalationSection.locator("form");
  await escalationForm
    .getByLabel("Политика SLA")
    .selectOption({ label: policyName });
  await escalationForm.getByLabel("Порог, %").fill("73");
  await escalationForm.getByLabel("Получатель").selectOption("specific_user");
  await escalationForm
    .getByLabel("Пользователь-получатель")
    .selectOption(managerRecipient!.id);
  await escalationForm
    .getByRole("button", { name: "Создать уведомление" })
    .click();
  const escalationCard = escalationSection
    .locator(".service-desk-sla-summary-card")
    .filter({ hasText: "Выполнение · 73%" });
  ...
  await escalationForm
    .getByRole("button", { name: "Сохранить уведомление" })
    .click();

  await page.reload();
  await expect(
    page.getByRole("heading", { name: `${calendarName} edited` }),
  ).toBeVisible();
  await expect(page.getByText("Выполнение · 81%")).toBeVisible();
  const calendars = await serviceDeskRequest<
    Array<{
      name: string;
  ...
  const calendar = calendars.find(
    (item) => item.name === `${calendarName} edited`,
  );
  expect(calendar?.business_hours).toHaveLength(2);
  expect(
    calendar?.exceptions.filter((item) => item.type === "custom_hours"),
  ).toHaveLength(2);
  const bindings = await serviceDeskRequest<
    Array<{
      name: string;
      priority: number;
      conditions: Array<{ field: string; field_key?: string }>;
    }>
  ...
  const persistedEscalationCard = escalationSection
    .locator(".service-desk-sla-summary-card")
    .filter({ hasText: "Выполнение · 81%" });
  const deleteEscalationResponse = page.waitForResponse(
    (response) =>
      response.request().method() === "DELETE" &&
      response.url().includes("/admin/sla/escalations/"),
  );
  await persistedEscalationCard
...
test("Service Desk Workbench assigns, starts and resolves a ticket", async ({
  page,
  request,
}) => {
  const token = await loginAsManager(page);
  const assignees = await serviceDeskRequest<
    Array<{ id: string; display_name: string }>
  >(request, token, "get", "/workbench/users?eligible_assignees=true");
  const employee = assignees.find(
    (item) => item.display_name === "Менеджер Service Desk",
  );
  ...
  const row = page
    .getByRole("row")
    .filter({ hasText: `E2E Workbench ticket ${suffix}` });
  ...
  await expect(
    row.getByText(employee?.display_name ?? "Менеджер Service Desk"),
  ).toBeVisible();
  ...
  const employeeRow = page
    ...
    .filter({ hasText: `E2E Workbench ticket ${suffix}` });
  await employeeRow
    .getByLabel(`Действия ${ticket.number}`)
    .selectOption("start");
...
test("Service Desk complete requester lifecycle runs through catalog and ticket UI", async ({
  page,
  request,
}) => {
  const token = await loginAsManager(page);
  const manager = await serviceDeskRequest<{ id: string }>(
    request,
    token,
    "get",
    "/me",
  ...
  const service = await serviceDeskRequest<{ id: string; title: string }>(
    request,
    token,
    "post",
    "/admin/services",
    {
      ...
      title: serviceTitle,
      short_description: "Услуга для полного UI сценария",
    },
  );
  const version = await serviceDeskRequest<{ id: string }>(
    request,
  ...
  const configured = await serviceDeskRequest<{ workflow: { id: string } }>(
    ...
    `/admin/template-versions/${version.id}/approval-workflow`,
    {
      ...
      name: "E2E пользовательское согласование",
      is_active: true,
    },
  );
  await serviceDeskRequest(
    ...
    "post",
    `/admin/approval-workflows/${configured.workflow.id}/stages`,
    { title: "Согласование менеджера", decision_rule: "any" },
  );
  const workflow = await serviceDeskRequest<{
  ...
  await serviceDeskRequest(
    ...
    token,
    "post",
    `/admin/approval-stages/${workflow.workflow.stages[0].id}/approvers`,
    { service_desk_user_id: manager.id },
  );
  await serviceDeskRequest(
    request,
    token,
  ...
  await page
    .getByLabel("Описание")
    .fill("Заявка создана через пользовательский каталог Service Desk.");
  await page.getByRole("button", { name: "Отправить заявку" }).click();
  await expect(page).toHaveURL(/\/service-desk\/tickets\//);
  await expect(
    page.getByRole("heading", { name: `E2E полный путь ${suffix}` }),
  ).toBeVisible();
  ...
  await page
    .getByLabel("Комментарий")
    .fill("Адрес объекта: главный корпус, кабинет 204.");
  ...
  await dialog
    .getByLabel("Итог выполнения")
    .fill("Запрос выполнен в полном объёме.");
  await dialog.getByRole("button", { name: "Подтвердить" }).click();
...
test("Service Desk seeded water form saves, resumes and submits with dictionary labels", async ({
  page,
}) => {
  await loginAsManager(page);
  ...
  await page
    .getByLabel("Дата и время начала мероприятия")
    .fill("2027-01-15T09:00");
  await page
    .getByLabel("Дата и время окончания мероприятия")
    .fill("2027-01-15T12:00");
  ...
  await page
    .getByLabel("Описание")
    .fill("Проверка сохранения формы из каталога.");
