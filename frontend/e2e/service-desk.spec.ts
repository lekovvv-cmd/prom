...
test("Service Desk SLA admin persists complete calendar, policy, binding and escalation edits", async ({ page, request }) => {
  ...
  await calendarForm.locator(".service-desk-sla-advanced-settings summary").click();
  ...
  await calendarForm.getByLabel("Название календаря").fill(`${calendarName} edited`);
  ...
  await policyForm.locator(".service-desk-sla-advanced-settings summary").click();
  await policyForm.getByLabel("Описание для команды").fill("E2E critical policy");
  await policyForm.getByLabel("Рабочий календарь").selectOption({ label: `${calendarName} edited` });
  await policyForm.getByLabel("Срок первого ответа, минут").fill("15");
  await policyForm.getByLabel("Срок решения, минут").fill("240");
  await policyForm.getByLabel("Останавливать срок, когда ждём заявителя").check();
  ...
  await bindingForm.getByLabel("Применять политику").selectOption({ label: policyName });
  ...
  await bindingForm.getByRole("button", { name: "Ещё условие", exact: true }).click();
  ...
  await bindingForm.locator(".service-desk-sla-advanced-settings summary").click();
  ...
  await escalationSection.locator(".service-desk-sla-advanced-settings summary").click();
