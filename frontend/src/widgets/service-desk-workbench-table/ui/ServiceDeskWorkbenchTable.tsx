...
export function ServiceDeskWorkbenchTable({ items, onAction }: { items: WorkbenchTicket[]; onAction: (ticket: WorkbenchTicket, action: ServiceDeskAllowedAction) => void }) {
  return <div className="service-desk-workbench-table-shell"><Table><table className="service-desk-workbench-table"><thead><tr><th>Номер</th><th>Заявка</th><th>Заявитель</th><th>Исполнитель</th><th>Приоритет</th><th>Статус</th><th>SLA</th><th>Создана</th><th>Обновлена</th><th>Действия</th></tr></thead>
    ...
    </tr>)}</tbody></table></Table><p className="workbench-table-scroll-hint"><ArrowRight size={15} aria-hidden="true" />Прокрутите таблицу, чтобы увидеть все столбцы</p></div>;
}
