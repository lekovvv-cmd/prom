...
const statuses: ProjectResponseStatus[] = ["new", "viewed", "contacted", "accepted", "rejected", "cancelled"];
...
export function ResponseStatusSelect({
  ...
  return (
    <Select
      ...
      value={value}
      onChange={(event) => void handleChange(event.target.value as ProjectResponseStatus)}
    >
      {statuses.map((status) => (
