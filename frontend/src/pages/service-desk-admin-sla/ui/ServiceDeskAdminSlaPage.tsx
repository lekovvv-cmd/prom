...
function SlaWorkflowOverview() {
  return <ol className="service-desk-sla-workflow" aria-label="Как настроить SLA">
    <li><strong>1. Календарь</strong><span>Задайте рабочее время.</span></li>
    <li><strong>2. Политика</strong><span>Укажите сроки и паузы.</span></li>
    <li><strong>3. Правило</strong><span>Свяжите SLA с заявками.</span></li>
    <li><strong>4. Эскалация</strong><span>Настройте предупреждения.</span></li>
  </ol>;
}
  ...
  return (
    <Card className="service-desk-sla-editor-card">
      ...
      <form className="service-desk-sla-form" onSubmit={onSubmit}>
        ...
        <div className="service-desk-sla-form-grid">
          <Input label="Первый ответ, минут" type="number" min="1" value={draft.firstResponseMinutes} onChange={(event) => onChange((current) => ({ ...current, firstResponseMinutes: event.target.value }))} required />
          <Input label="Выполнение, минут" type="number" min="1" value={draft.resolutionMinutes} onChange={(event) => onChange((current) => ({ ...current, resolutionMinutes: event.target.value }))} required />
        </div>
        <div className="service-desk-sla-check-list">
          ...
          <ActiveCheckbox checked={draft.isActive} label="Политика активна" onChange={(isActive) => onChange((current) => ({ ...current, isActive }))} />
        </div>
        <FormActions editingId={editingId} isSaving={isSaving} onCancel={onCancel} />
      </form>
    </Card>
  ...
  return (
    <Card className="service-desk-sla-editor-card">
      ...
      <form className="service-desk-sla-form" onSubmit={onSubmit}>
        <Input label="Название" value={draft.name} onChange={(event) => onChange((current) => ({ ...current, name: event.target.value }))} required />
        <div className="service-desk-sla-form-grid">
          <Select label="Политика SLA" value={draft.policyId} onChange={(event) => onChange((current) => ({ ...current, policyId: event.target.value }))} required><option value="">Выберите политику</option>{policies.map((policy) => <option key={policy.id} value={policy.id}>{policy.name}</option>)}</Select>
        ...
        <ActiveCheckbox checked={draft.isActive} label="Правило активно" onChange={(isActive) => onChange((current) => ({ ...current, isActive }))} />
        <div className="service-desk-sla-form-section">
