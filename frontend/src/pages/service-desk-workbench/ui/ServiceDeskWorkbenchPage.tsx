...
export function ServiceDeskWorkbenchPage() {
  const [filters, setFilters] = useState<Record<string, string>>({ page: "1", page_size: "25" }); const [search, setSearch] = useState("");
  const [data, setData] = useState<{ items: WorkbenchTicket[]; total: number; pages: number } | null>(null); const [counters, setCounters] = useState<WorkbenchCounters | null>(null);
  const [users, setUsers] = useState<WorkbenchUserOption[]>([]); const [assignees, setAssignees] = useState<WorkbenchUserOption[]>([]); const [categories, setCategories] = useState<CatalogOption[]>([]); const [services, setServices] = useState<CatalogOption[]>([]);
  const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [action, setAction] = useState<{ ticket: WorkbenchTicket; action: ServiceDeskAllowedAction } | null>(null); const [payload, setPayload] = useState(""); const [pending, setPending] = useState(false);
  ...
  return <><Header /><PageLayout title="Рабочее место Service Desk" subtitle="Операционная очередь заявок, согласований и SLA">
    ...
    <Card><div className="filters workbench-filters"><Input label="Поиск" value={search} onChange={(e) => { setSearch(e.target.value); setFilters((current) => ({ ...current, page: "1" })); }} placeholder="Номер, название или услуга" />
      ...
      <Select label="Категория" value={filters.category_id ?? ""} onChange={(e) => { update("category_id", e.target.value); setFilters((current) => ({ ...current, service_id: "", category_id: e.target.value, page: "1" })); }}><option value="">Все</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.title}</option>)}</Select>
      <Select label="Услуга" value={filters.service_id ?? ""} onChange={(e) => update("service_id", e.target.value)}><option value="">Все</option>{services.filter((s) => !filters.category_id || s.category_id === filters.category_id).map((s) => <option key={s.id} value={s.id}>{s.title}</option>)}</Select>
      <Select label="SLA" value={filters.sla_state ?? ""} onChange={(e) => update("sla_state", e.target.value)}><option value="">Все</option><option value="no_sla">Без SLA</option><option value="on_track">В норме</option><option value="paused">На паузе</option><option value="warning">Риск</option><option value="breached">Нарушен</option></Select>
      <Select label="Просрочка" value={filters.overdue ?? ""} onChange={(e) => update("overdue", e.target.value)}><option value="">Все</option><option value="true">Только просроченные</option><option value="false">Без просрочки</option></Select>
      <Input label="Создана с" type="datetime-local" value={filters.created_from ?? ""} onChange={(e) => update("created_from", e.target.value)} /><Input label="Создана по" type="datetime-local" value={filters.created_to ?? ""} onChange={(e) => update("created_to", e.target.value)} />
      <Button variant="ghost" onClick={() => { setFilters({ page: "1", page_size: "25" }); setSearch(""); }}>Сбросить</Button></div></Card>
    {error && <p className="form-error" role="alert">{error}</p>}{loading ? <Spinner label="Загружаем заявки" /> : data?.items.length ? <><ServiceDeskWorkbenchTable items={data.items} onAction={(ticket, selected) => { setAction({ ticket, action: selected }); setPayload(""); }} /><div className="workbench-pagination"><Button variant="secondary" disabled={filters.page === "1"} onClick={() => update("page", String(Number(filters.page) - 1))}>Назад</Button><span>Страница {filters.page} из {data.pages || 1} · {data.total} заявок</span><Button variant="secondary" disabled={Number(filters.page) >= data.pages} onClick={() => update("page", String(Number(filters.page) + 1))}>Далее</Button></div></> : <EmptyState title={hasFilters ? "По выбранным фильтрам ничего не найдено" : "В рабочем месте пока нет заявок"} text={hasFilters ? "Измените или сбросьте фильтры." : "Новые операционные заявки появятся здесь."} />}
  </PageLayout>{action && <Modal title={quickLabels[action.action as WorkbenchQuickView] ?? "Действие с заявкой"} onClose={() => setAction(null)}><div className="modal-body">{action.action === "assign" || action.action === "reassign" ? <Select label="Исполнитель" value={payload} onChange={(e) => setPayload(e.target.value)}><option value="">Выберите</option>{assignees.map((u) => <option key={u.id} value={u.id}>{u.display_name}</option>)}</Select> : payloadFields[action.action] ? <label className="field"><span>{payloadFields[action.action]?.[1]}</span><textarea value={payload} onChange={(e) => setPayload(e.target.value)} /></label> : <p>Подтвердите действие для заявки {action.ticket.number}.</p>}<div className="table-actions"><Button disabled={pending || ((action.action === "assign" || action.action === "reassign") && !payload)} onClick={() => void submitAction()}>{pending ? "Выполняем…" : "Подтвердить"}</Button><Button variant="secondary" onClick={() => setAction(null)}>Отмена</Button></div></div></Modal>}</>;
}
