...
export function ServiceDeskMyTicketsPage() {
  ...
  const visible = useMemo(() => tickets.filter((ticket) => matchesFilter(ticket, filter)).filter((ticket) => !query.trim() || `${ticket.number ?? ""} ${ticket.title} ${ticket.service.title}`.toLowerCase().includes(query.trim().toLowerCase())), [filter, query, tickets]);
