...
const emptyFilters: AccessFilters = { q: "", accessType: "", isActive: "", page: 1 };

function params(filters: AccessFilters) {
  const value = new URLSearchParams({ page_size: "10", page: String(filters.page) });
...
export function ServiceDeskAdminAccessPage() {
  ...
  async function mutate(action: () => Promise<ServiceDeskUser>, targetUserId?: string) {
    setPending(true);
    setError(null);
    try {
      await action();
  ...
  async function applyActiveChange() {
    ...
    setConfirming(null);
    await mutate(() => setAccessUserActive(target.id, !target.is_active), target.id);
  }
  ...
  return (
    <>
      <PageLayout title="Менеджеры и права">
        ...
        <Card>
          <div className="filter-grid">
            ...
            <Button variant="ghost" onClick={() => setFilters(emptyFilters)}>Сбросить</Button>
          </div>
          ...
          <Table>
            <table>
              <thead>
                <tr><th>Пользователь</th><th>Тип</th><th>Права</th><th>Статус</th><th>Действия</th></tr>
              </thead>
              <tbody>
                {users.map((user) => (
                  <tr key={user.id}>
                    <td><strong>{user.display_name}</strong><br /><small>{user.email} · {user.department || "-"} · {user.position || "-"}</small></td>
                    <td>{user.access_type === "service_desk_admin" ? "Администратор SD" : "Менеджер"}</td>
                    <td>
                    ...
                    <td>
                      <div className="table-actions">
                        <Button variant="secondary" disabled={pending} onClick={() => setEditing(user)}>Изменить</Button>
                        <Button variant="secondary" disabled={pending} onClick={() => setConfirming(user)}>{user.is_active ? "Деактивировать" : "Активировать"}</Button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="workbench-pagination">
            <Button variant="secondary" disabled={filters.page <= 1} onClick={() => updateFilter("page", filters.page - 1)}>Назад</Button>
            <span>Страница {filters.page} из {pages} · {total}</span>
            <Button variant="secondary" disabled={filters.page >= pages} onClick={() => updateFilter("page", filters.page + 1)}>Далее</Button>
          </div>
        </Card>
      </PageLayout>

      {editing && (
        <Modal title="Изменить профиль доступа" onClose={() => setEditing(null)}>
          <form className="modal-body" onSubmit={(event) => void saveEdit(event)}>
            <Input name="email" label="Email" type="email" required defaultValue={editing.email} />
            <Input name="name" label="Имя" required defaultValue={editing.display_name} />
      ...
      {confirming && (
        <Modal title={confirming.is_active ? "Деактивировать доступ" : "Активировать доступ"} onClose={() => setConfirming(null)}>
