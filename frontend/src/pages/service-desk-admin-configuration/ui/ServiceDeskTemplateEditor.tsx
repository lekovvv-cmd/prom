...
function VersionSettings({ version, onSaved }: { version: AdminTemplateVersion; onSaved: (version: AdminTemplateVersion) => void }) {
  ...
  return <Card className="template-version-settings"><h3>Настройки версии</h3><div className="admin-config-form-grid"><Input label="Тема по умолчанию" value={defaultTitle} onChange={(event) => setDefaultTitle(event.target.value)} /><label className="checkbox-field"><input type="checkbox" checked={titleEditable} onChange={(event) => setTitleEditable(event.target.checked)} />Тему можно изменять</label><label className="checkbox-field"><input type="checkbox" checked={descriptionRequired} onChange={(event) => setDescriptionRequired(event.target.checked)} />Описание обязательно</label><Input label="Подсказка формы" value={helpText} onChange={(event) => setHelpText(event.target.value)} /><Select label="Исполнитель по умолчанию" value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}><option value="">Не назначен</option>{assignees.map((user) => <option key={user.id} value={user.id}>{user.display_name}</option>)}</Select><Button onClick={() => void save()}><Save size={15} />Сохранить настройки</Button></div></Card>;
}

function ConditionLists({ draft, fieldKeys, onChange }: { draft: EditorDraft; fieldKeys: string[]; onChange: (draft: EditorDraft) => void }) {
  const visibility = ruleArray(draft.preserved_visibility_rules);
  const required = ruleArray(draft.preserved_required_rules);
  const setRules = (kind: "visibility" | "required", rules: Rule[]) => onChange({ ...draft, [kind === "visibility" ? "preserved_visibility_rules" : "preserved_required_rules"]: rules });
  return <Card><h3>Условия поля</h3><p className="muted">Все условия в каждом списке применяются одновременно (AND).</p><RuleList title="Условия видимости" rules={visibility} fieldKeys={fieldKeys} ownKey={draft.key} onChange={(rules) => setRules("visibility", rules)} /><RuleList title="Условия обязательности" rules={required} fieldKeys={fieldKeys} ownKey={draft.key} onChange={(rules) => setRules("required", rules)} /></Card>;
}

function RuleList({ title, rules, fieldKeys, ownKey, onChange }: { title: string; rules: Rule[]; fieldKeys: string[]; ownKey: string; onChange: (rules: Rule[]) => void }) {
  const available = fieldKeys.filter((key) => key !== ownKey);
  const update = (index: number, patch: Partial<Rule>) => onChange(rules.map((rule, ruleIndex) => ruleIndex === index ? { ...rule, ...patch } : rule));
  return <section><div className="service-desk-section-heading"><h4>{title}</h4><Button variant="secondary" onClick={() => onChange([...rules, { field: available[0] ?? "", operator: "equals", value: "" }])}><Plus size={14} />Добавить условие</Button></div>{rules.map((rule, index) => { const operator = String(rule.operator ?? "equals"); const unknownField = Boolean(rule.field) && !fieldKeys.includes(String(rule.field)); return <div className="admin-config-form-grid" key={`${title}-${index}`}><Select label="Поле" value={String(rule.field ?? "")} onChange={(event) => update(index, { field: event.target.value })}><option value="">Выберите поле</option>{available.map((key) => <option key={key} value={key}>{key}</option>)}{unknownField ? <option value={String(rule.field)}>Неизвестное: {String(rule.field)}</option> : null}</Select><Select label="Оператор" value={operator} onChange={(event) => update(index, { operator: event.target.value })}>{operators.map((item) => <option key={item} value={item}>{operatorLabel(item)}</option>)}</Select>{!["is_empty", "is_not_empty"].includes(operator) ? <Input label="Значение" value={String(rule.value ?? "")} onChange={(event) => update(index, { value: event.target.value })} /> : null}<Button variant="ghost" onClick={() => onChange(rules.filter((_, ruleIndex) => ruleIndex !== index))}><Trash2 size={14} />Удалить</Button>{String(rule.field ?? "") === ownKey ? <small className="field-error">Поле не может ссылаться само на себя.</small> : null}{unknownField ? <small className="field-error">Поле с таким ключом не существует.</small> : null}</div>; })}</section>;
}

function ruleArray(value: Rule | Rule[] | null) {
  if (!value) return [];
...
function fieldTypeLabel(type: string) { const labels: Record<string, string> = { text: "Текст", textarea: "Большой текст", rich_text: "Многострочный текст", select: "Список", multiselect: "Несколько значений", date: "Дата", datetime: "Дата и время", email: "Email", number: "Число", checkbox: "Флажок", file: "Файл", user: "Пользователь" }; return labels[type] ?? "Поле"; }
